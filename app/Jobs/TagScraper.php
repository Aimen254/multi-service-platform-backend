...
class TagScraper implements ShouldQueue
{
    ...
    private function assignMissingTags($newProduct)
    {
        ...
        $productL2Tag = $newProduct->standardTags()->whereHas('levelTwo')->first();
        $productL3Tag = $newProduct->standardTags()->whereHas('levelThree')->first();
        $productL4Tags = $newProduct->standardTags()->whereHas('tagHierarchies', function ($query) {
        ...
        if (empty($productL2Tag)) {
            ...
            if (!empty($productL3Tag)) {
                $l2Tags = StandardTag::whereHas('levelTwo', function ($query) use ($productL3Tag) {
                    $query->where('L3', $productL3Tag->id);
                })->get()->toArray();
            }
            ...
            // otherwise, we use level one tag to get level two tags
                ...
                $l2Tags = $newProduct->business->standardTags()->whereHas('levelTwo')->get()->toArray();
            ...
            // otherwise, we ask AI to suggest a tag from the given list
                ...
                $aiSuggestedTag = $chatGPTService->getCategory(array_column($l2Tags, 'name'), $newProduct->name);
        ...
        if (empty($productL3Tag)) {
            ...
            if ($productL4Tags->count() > 0) {
                $l3Tags = StandardTag::whereHas('levelThree', function ($query) use ($productL4Tags) {
            ...
            // otherwise, we use level two tag to get level three tags
                ...
                $l3Tags = $newProduct->business->standardTags()->whereHas('levelThree')->get()->toArray();
                ...
                if (count($l3Tags) <= 0) {
                    // get all the level two tags which has been specified by the business
                    $l2TagsIds = $newProduct->business->standardTags()->whereHas('levelTwo')->pluck('id');
                    ...
                    $l3Tags = StandardTag::whereHas('levelThree', function ($query) use ($l2TagsIds) {
                        $query->whereIn('L2', $l2TagsIds);
                    })->get()->toArray();
                }
            }

            $aiSuggestedTag = $chatGPTService->getCategory(array_column($l3Tags, 'name'), $newProduct->name);
        ...
        if ($productL4Tags->count() <= 0) {
            ...
            $l4Tags = StandardTag::whereHas('tagHierarchies', function ($query) use ($productL3Tag) {
                $query->where('level_type', 4)->where('L3', $productL3Tag->id);
            })->get()->toArray();

            $aiSuggestedTag = $chatGPTService->getCategory(array_column($l4Tags, 'name'), $newProduct->name);

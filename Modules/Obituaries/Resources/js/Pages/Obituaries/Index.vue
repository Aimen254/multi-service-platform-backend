<template>
    <Head title="Obituaries" />
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Obituaries`" :path="`Obituaries`"></Breadcrumbs>
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <div class="me-4">
                    ...
                    <Filter :filterData="filterForm" :callType="type" @removeSelectedTags="removeSelectedTags()" :business="obituaries" :url="urlForFilter"
                ...
                <Link v-if="checkUserPermissions('add_products')"
                    :href="route('obituaries.dashboard.obituaries.create', this.getSelectedModuleValue())"
        ...
        <div :class="widgetClasses" class="card">
            ...
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bolder fs-3 mb-1">
                        <ObituariesSearchInput :callType="type" :searchedKeyword="searchedKeyword" />
                    </span>
                </h3>
            </div>
            <!--end::Header-->
                    ...
                    <table class="table align-middle gs-0 gy-4 table-row-dashed">
                        ...
                        <thead>
                            <tr class="fw-bolder text-muted bg-light">
                                <th class="ps-4 min-w-130px rounded-start">Title</th>
                                <th class="ps-4 min-w-130px rounded-start">Total Comments</th>
                                <th class="ps-4 min-w-130px rounded-start">Date of Birth</th>
                                <th class="ps-4 min-w-130px rounded-start">Date of Death</th>
                                <th class="min-w-130px">Status</th>
                                <th class="min-w-130px rounded-end">Action</th>
                            </tr>
                        </thead>
                        <!--end::Table head-->
                        <!--begin::Table body-->
                        <tbody v-if="obituaries && obituaries.data.length > 0">
                            <template v-for="item in obituaries.data" :key="item.id">
                                <tr>
                                    <td class="px-4">
                                        <div class="d-flex align-items-center">
                                            <div class="image-input image-input-empty">
                                                <div class="image-input-wrapper w-50px h-50px"
                                                    :style="{ 'background-image': 'url(' + getImage(item.main_image ? item.main_image.path : null, true, 'obituaries', item.main_image ? item.main_image.is_external : 0) + ')' }">
                                                </div>
                                            </div>
                                    ...
                                    <td>
                                        <span class="text-muted fw-bold text-muted d-block text-capitalize fs-7">{{
                                            formatDate(item.date_of_birth) }}</span>
                                    </td>
                                    <td>
                                        <span class="text-muted fw-bold text-muted d-block text-capitalize fs-7">{{
                                            formatDate(item.date_of_death) }}</span>
                                    </td>
                                    <td>
                                        <span
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            ...
                                            <view-section permission="view_products"
                                                :url="route('obituaries.dashboard.obituaries.show', [getSelectedModuleValue(), item.uuid])" />
                                                <!-- :url="route('obituaries.dashboard.obituaries.comments.index', [getSelectedModuleValue(), item.uuid])"  -->
                                            <edit-section iconType="link" permission="edit_products"
                                                :url="route('obituaries.dashboard.obituaries.edit', [getSelectedModuleValue(), item.uuid])" />
                                            <delete-section permission="delete_products"
                                                :url="route('obituaries.dashboard.obituaries.destroy', [getSelectedModuleValue(), item.uuid])" 
                                                :currentPage="obituaries.current_page" :currentCount="obituaries.data.length"/>
                                        </div>
                                    </td>
                                </tr>
                            </template>
            ...
            <!--begin::Body-->
            <pagination :meta="obituaries" :keyword="searchedKeyword" />
        </div>
...
import ObituariesSearchInput from "../../Components/ObituariesSearchInput.vue";
...
export default {
    components: {
        ...
        Breadcrumbs,
        Toggle,
        ObituariesSearchInput
    },

    props: ["obituariesList", "searchedKeyword", "orderBy", "status", "tag"],

    data() {
        return {
            obituaries: this.obituariesList,
            type: "obituaries",
            module: 'obituaries',
            urlForFilter: 'obituaries.dashboard.obituaries.index',
            filterForm: {
    ...
    watch: {
        obituariesList: {
            handler(obituariesList) {
                this.obituaries= obituariesList;
            },
            deep: true,
        },
    },
        ...
        changeStatus(uuid) {
            this.swal.fire({
                ...
                if (result.value) {
                    ...
                    this.$inertia.visit(route('obituaries.dashboard.obituaries.status', [this.getSelectedModuleValue(), uuid]), {
        ...
        getTags() {
            ...
            setTimeout(() => {
                $(".select").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("obituaries.dashboard.search.obituaries.tags", parameters),
                        dataType: "json",
                        delay: 250,
    ...
    mounted() {
        // console.log(this.obituariesList);
        this.getTags();
    },
    mixins: [Helpers],
};

<template>
    <Head title="Obituary Images" />
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Obituary Images`" :path="`Obituary - ${limit(obituaries?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <ProductSidebar :obituaries="obituaries" :width="'w-lg-225px'"/>
                        ...
                        <div class="fv-row mb-4">
                            <div class="image-input image-input-outline image-input-empty" data-kt-image-input="true">
                                <div class="image-input-wrapper w-125px h-125px"
                                    :style="{ 'background-image': 'url(' + getImage(url, isSaved, 'obituaries', obituaries.main_image ? obituaries.main_image.is_external : 0) + ')' }"></div>
    ...
    methods : {
        ...
        submit() {
            ...
            this.$inertia.post(route("obituaries.dashboard.obituaries.media.update", [this.getSelectedModuleValue(), this.obituaries.uuid, this.obituaries.main_image ? this.obituaries.main_image.id : 0]), this.form, {
                errorBag: "admin",
                preserveScroll: true,
                onError: (data) => {
    ...
    mounted(){
        var productMedia = this.obituaries
        this.form = useForm({
            image: "",
        });
        ...
        var submissionUrl = route('obituaries.dashboard.obituaries.media.store', [moduleId, this.obituaries.uuid])
        $("#product").dropzone({
            url: submissionUrl,
            parallelUploads:6,
            ...
            removedfile: function (file) {
                if (file.id) {
                    window.swal.fire({
                        ...
                        if (result.value) {
                            showWaitDialog()
                            window.axios.delete(route('obituaries.dashboard.obituaries.media.destroy', [moduleId, productMedia.uuid, file.id]))
                                .then((response) => {
                                    hideWaitDialog()

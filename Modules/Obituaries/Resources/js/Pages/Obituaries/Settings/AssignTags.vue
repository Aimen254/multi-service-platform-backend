<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Assign Tags`" :path="`Obituary - ${limit(obituaries?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <ProductSidebar :obituaries="obituaries" style="width: 25%!important;" />
                    ...
                    <div class="card card-flush py-4">
                        <div class="card-body py-3 row">
                            ...
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Tags" />
                                <TagComponent placeholder="Tags" :uuid="obituaries.uuid" :assignedTags="productTags" :onlyFromAutoComplete="true" :openOnClick="false" :tooltipContent="'Mapped Tags Information.'"/>
                                <error :message="form.errors.tags" />
                            </div>
                            <!-- Extra -->
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Extra Tags" />
                                <TagComponent placeholder="Extra Tags" :uuid="obituaries.uuid" :assignedTags="extraTag" :autocomplete="filteredAllTags" :onlyFromAutoComplete="false" :openOnClick="false" :tooltipContent="'Unmapped Tags Information.'"/>
                                <error :message="form.errors.tags" />
                            </div>
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Ignored Tags" />
                                <TagComponent placeholder="Ignored Tags" :uuid="obituaries.uuid" :assignedTags="ignoredTags" :autocomplete="allIgnoredAutocomplete" :onlyFromAutoComplete="false" :openOnClick="true"   :tooltipContent="'Ignored Inappropriate Tags.'" />
                                <error :message="form.errors.tags" />
                            </div>
...
export default {
    props: ['obituaries', 'allTags', 'allproductTags', 'extraTags', 'allIgnoredAutocomplete', 'productIgnoredTags'],
    ...
    methods: {
        submit () {
            // let tags  = this.news.tags.concat(this.levelOneTag)
            ...
            this.form.get(route("obituaries.dashboard.obituaries.tags.assign", [this
                .getSelectedModuleValue(), this.obituaries.uuid
            ]));
        },
        allTagsFn() {

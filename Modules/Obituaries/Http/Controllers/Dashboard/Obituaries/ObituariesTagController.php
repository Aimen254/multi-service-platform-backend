...
class ObituariesTagController extends Controller
{
    /**
    ...
    public function index($moduleId, $uuid)
    {
        try {
            ...
            // Getting orphan tags
            $orphanTags = $obituaries->tags()->asTag()->where(function ($query) {
            ...
            $extraOrphanTags = $obituaries->tags()->asTag()->whereDoesntHave('standardTags_')->get();
            // Getting extra standard tags
            $extraStandardTags = $obituaries->standardTags()->asTag()->wherePriority(4)->whereHas('tags_')->get();
            // Getting tags
            $extraTags = Arr::collapse([$extraOrphanTags, $extraStandardTags]);
            // Getting ignored orphan tags
            $ignoredOrphanTags = $obituaries->tags()->asTag()->whereHas('standardTags_', function ($query) {
                $query->where('priority', '<>', 1);
            })->orWhereDoesntHave('standardTags_')->where('product_id', $obituaries->id)->get()->reject(function ($record) {
                $tag = $record->whereHas('standardTags_', function ($query) use ($record) {
                    $query->where('priority', '<>', 1)->where('slug', $record->slug);
                })->first();
                return $tag ? true : false;
            ...
            $ignoredTags = $obituaries->ignoredTags()->first();
            $productIgnoredTags = [];
            if ($ignoredTags) {
            ...
            $ignoredStandardTags = $obituaries->standardTags()->asTag()->where('type', '<>', 'module')->where('priority', '<>', 1)->get();

            return inertia('Obituaries::Obituaries/Settings/AssignTags', [
    ...
    private function storeStandardTags($obituaries, $standardTags): void
    {
        foreach ($standardTags as $id) {
            ...
            if ($existInBothLevels) {
                $productLevelThreeTag = $obituaries->standardTags()->where('id', '<>', $standardTag->id)->whereHas('levelTHree')->first();
                if ($productLevelThreeTag) {
                    ...
                    if ($exsistInlevelFour) {
                        $obituaries->standardTags()->syncWithOutDetaching($standardTag->id);
                    }
                }
            } else {
                $obituaries->standardTags()->syncWithOutDetaching($standardTag->id);
    ...
    public function assignTags($moduleId, $uuid)
    {
        try {
            ...
            $obituaries = Product::whereUuid($uuid)->firstOrFail();
            DB::beginTransaction();
            $tags = $this->handleTags($moduleId, request()->input('tags'), $obituaries);
            $categoryTags = collect(json_decode(request()->input('categoryTags')))->pluck('id')->toArray();
            $ignoredTagIds = $this->productIgnoredTags(request()->ignoredTags, $obituaries);
            $productCategoryTags = $obituaries->tags()->asTag()->where(function ($query) {
                $query->where('is_category', true)->whereHas('standardTags_', function ($query) {
                    $query->where('priority', 1);
                });
            })->get()->reject(function ($record) {
            ...
            $obituaries->tags()->sync($orphanTags);
            $standardTagIds =  $obituaries->standardTags()->whereNotIn('type', ['module', 'attribute'])->wherePriority(4)->pluck('id')->toArray();
            ...
            if (request()->input('removeStandardTags')) {
                ...
                $obituaries->standardTags()->detach(request()->input('removeStandardTags'));
            }

            $removeTags = array_diff($standardTagIds, $standardTags);
            $obituaries->standardTags()->detach($removeTags);

            $this->storeStandardTags($obituaries, $standardTags);

            $obituaries->tags()->detach($ignoredTagIds['ignoredOrphanTagIds']);
            $obituaries->standardTags()->detach($ignoredTagIds['ignoredStandardTagIds']);
            ProductTagsLevelManager::checkProductTagsLevel($obituaries);
            ProductTagsLevelManager::priorityOneTags($obituaries, $removeCategoryTags, 'product_tag');
            ProductTagsLevelManager::priorityFour($obituaries, $tags['orphanTags']);
            ProductTagsLevelManager::priorityTwoTags($obituaries);
            DB::commit();
            flash('Obituaries tags updated successfully.', 'success');
            return \redirect()->back();

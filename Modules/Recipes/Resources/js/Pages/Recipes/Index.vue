<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Recipes`" :path="`Recipes`"></Breadcrumbs>
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <div class="me-4">
                    ...
                    <Filter :filterData="filterForm" :callType="type" @removeSelectedTags="removeSelectedTags()"
                        :business="recipes" :url="urlForFilter" :newKeyword="searchedKeyword">
                        <div class="mb-5">
                ...
                <Link v-if="checkUserPermissions('add_products')"
                    :href="route('recipes.dashboard.recipes.create', this.getSelectedModuleValue())"
        ...
        <div :class="widgetClasses" class="card">
            ...
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bolder fs-3 mb-1">
                        <RecipesSearchInput :callType="type" :searchedKeyword="searchedKeyword" />
                    </span>
                </h3>
            </div>
            <!--end::Header-->
            ...
            <div class="card-body py-3">
                ...
                <div class="table-responsive">
                    ...
                    <table class="table align-middle gs-0 gy-4 table-row-dashed">
                        ...
                        <!--end::Table head-->
                        <!--begin::Table body-->
                        <tbody v-if="recipes && recipes.data.length > 0">
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            ...
                                            <view-section permission="view_products"
                                                :url="route('recipes.dashboard.recipes.show', [getSelectedModuleValue(), item.uuid])" />
                                            <edit-section iconType="link" permission="edit_products"
                                                :url="route('recipes.dashboard.recipes.edit', [getSelectedModuleValue(), item.uuid])" />
                                            <delete-section permission="delete_products"
                                                :url="route('recipes.dashboard.recipes.destroy', [getSelectedModuleValue(), item.uuid])" 
            ...
            <!--begin::Body-->
            <pagination :meta="recipes" :keyword="searchedKeyword" />
        </div>
        <!--end::Tables Widget 11-->
        <HeadlineModal :url="route('recipes.dashboard.headlines.create', getSelectedModuleValue())" />
    </AuthenticatedLayout>
</template>
...
import RecipesSearchInput from "../../Components/RecipesSearchInput.vue";
...
export default {
    ...
    props: ["recipesList", "searchedKeyword", "orderBy", "status", "tag"],
    ...
    data() {
        return {
            ...
            type: "recipes",
            module: 'recipes',
            urlForFilter: 'recipes.dashboard.recipes.index',
            filterForm: {
    ...
    methods: {
        newsTitle(title, limit = 36) {
            var name = '';
            (title.length > limit
                ? name = title.slice(0, 36) + '...'
                : name = title)
            return name;
        },
        changeStatus(uuid) {
            this.swal.fire({
                ...
                if (result.value) {
                    ...
                    this.$inertia.visit(route('recipes.dashboard.recipes.status', [this.getSelectedModuleValue(), uuid]), {
        ...
        getTags() {
            ...
            setTimeout(() => {
                $(".select").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("recipes.dashboard.search.recipes.tags", parameters),
                        dataType: "json",
                        delay: 250,
        ...
        openModal(recipe) {
            this.emitter.emit("headline-modal", recipe);
        },
    },
    mounted() {
        this.getTags();

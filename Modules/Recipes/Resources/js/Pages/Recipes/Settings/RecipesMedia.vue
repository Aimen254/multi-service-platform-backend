<template>
<Head title="Recipe Images" />
<AuthenticatedLayout>
    <template #breadcrumbs>
        <Breadcrumbs :title="`Recipe Images`" :path="`Products - ${limit(recipes?.name, 36)}`" />
    </template>
    <div class="d-flex flex-column flex-lg-row">
        <ProductSidebar :recipes="recipes" :width="'w-lg-225px'"/>
                    ...
                    <div class="fv-row mb-4">
                        <div class="image-input image-input-outline image-input-empty" data-kt-image-input="true">
                            <div class="image-input-wrapper w-125px h-125px"
                                :style="{ 'background-image': 'url(' + getImage(url, isSaved, 'product', recipes.main_image ? recipes.main_image.is_external : 0) + ')' }"></div>
...
methods : {
    ...
    submit() {
        ...
        this.$inertia.post(route("recipes.dashboard.recipes.media.update", [this.getSelectedModuleValue(), this.recipes.uuid, this.recipes.main_image ? this.recipes.main_image.id : 0]), this.form, {
            errorBag: "admin",
            preserveScroll: true,
            onError: (data) => {
...
mounted(){
    ...
    var submissionUrl = route('recipes.dashboard.recipes.media.store', [moduleId, this.recipes.uuid])
    $("#product").dropzone({
        url: submissionUrl,
        parallelUploads:6,
        ...
        removedfile: function (file) {
            if (file.id) {
                window.swal.fire({
                    ...
                    if (result.value) {
                        showWaitDialog()
                        window.axios.delete(route('recipes.dashboard.recipes.media.destroy', [moduleId, productMedia.uuid, file.id]))
                            .then((response) => {
                                hideWaitDialog()

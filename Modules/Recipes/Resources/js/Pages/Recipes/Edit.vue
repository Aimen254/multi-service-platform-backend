<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Edit Recipe`" :path="`Recipes - ${limit(recipes?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <ProductSidebar :recipes="recipes" :width="'w-lg-820px'" />
                            ...
                            <div class="row mt-8">
                                <div class="col-lg-12 py-2">
                                    ...
                                    <Input id="name" type="text"
                                        ...
                                        autofocus autocomplete="name" placeholder="Enter recipes title" />
                                ...
                                <div class="col-lg-12 py-2">
                                    ...
                                    <QuillEditor theme="snow" v-model:content="form.description" contentType="html"
                                        placeholder='Compose recipes description...' />
...
export default {
    props: ['recipes', 'levelTwoTags', 'productLevelTwoTag', 'mediaSizes'],
    ...
    methods: {
        submit() {
            this.form.put(route('recipes.dashboard.recipes.update', [this.getSelectedModuleValue(), this.recipes.uuid]), {
                errorBag: "recipes",
                preserveScroll: true,
            });
        },
        getLevelThreeTags(tag, level) {
            ...
            setTimeout(() => {
                var select2 = $(".select").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("recipes.dashboard.recipes.tags", parameters),
                        dataType: "json",
                        delay: 250,
                ...
                // selected value
                window.axios.get(route('recipes.dashboard.recipes.tags', [this.getSelectedModuleValue(), tag, level]), {
                    params: {
    ...
    mounted() {
        this.form = this.$inertia.form({
            id: this.recipes ? this.recipes.id : null,
            name: this.recipes ? this.recipes.name : '',
            description: this.recipes ? this.recipes.description : '',
            ...
            is_featured: this.recipes.is_featured ? true : false,
            is_commentable: this?.recipes?.is_commentable ? true : false,

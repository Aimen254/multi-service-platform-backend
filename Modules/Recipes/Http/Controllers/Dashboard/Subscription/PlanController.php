...
namespace Modules\Recipes\Http\Controllers\Dashboard\Subscription;
...
class PlanController extends Controller
{
    ...
    public function index($moduleId)
    {
        ...
        return Inertia::render('Recipes::Subscription/Plans/Index', [
    ...
    {
        return Inertia::render('Recipes::Subscription/Plans/Edit', []);
    }
    ...
    public function update(subscriptionPlanRequest $request, $moduleId, $id)
    {
        try {
            ...
            return \redirect()->route('recipes.dashboard.subscription.plan.index', $moduleId);
        } catch (\Stripe\Exception\CardException $e) {
            flash($e->getMessage(), 'danger');
            return \back();
        } catch (\Stripe\Exception\InvalidRequestException $e) {
    ...
    public function destroy($moduleId, $id,Request $request)
    {
        try {
            ...
            if ($currentCount > 1) {
                ...
                return Redirect::route('recipes.dashboard.subscription.plan.index', [$moduleId, 'page' => $previousPage]);
            }
        } catch (\Stripe\Exception\CardException $e) {
            flash($e->getMessage(), 'danger');
            return \back();

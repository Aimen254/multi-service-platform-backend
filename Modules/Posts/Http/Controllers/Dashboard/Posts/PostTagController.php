...
class PostTagController extends Controller
{
    ...
    public function index($moduleId, $uuid)
    {
        try {
            ...
            // Getting orphan tags
            $orphanTags = $post->tags()->asTag()->where(function ($query) {
            ...
            $extraOrphanTags = $post->tags()->asTag()->whereDoesntHave('standardTags_')->get();
            // Getting extra standard tags
            $extraStandardTags = $post->standardTags()->asTag()->wherePriority(4)->whereHas('tags_')->get();
            // Getting tags
            $extraTags = Arr::collapse([$extraOrphanTags, $extraStandardTags]);
            // Getting ignored orphan tags
            $ignoredOrphanTags = $post->tags()->asTag()->whereHas('standardTags_', function ($query) {
                $query->where('priority', '<>', 1);
            })->orWhereDoesntHave('standardTags_')->where('product_id', $post->id)->get()->reject(function ($record) {
                $tag = $record->whereHas('standardTags_', function ($query) use ($record) {
                    $query->where('priority', '<>', 1)->where('slug', $record->slug);
                })->first();
                return $tag ? true : false;
            ...
            $ignoredTags = $post->ignoredTags()->first();
            $productIgnoredTags = [];
            if ($ignoredTags) {
            ...
            $ignoredStandardTags = $post->standardTags()->asTag()->where('type', '<>', 'module')->where('priority', '<>', 1)->get();

            return inertia('Posts::Posts/Settings/AssignTags', [
                'post' => $post,
    ...
    private function storeStandardTags($post, $standardTags): void
    {
        foreach ($standardTags as $id) {
            ...
            if ($existInBothLevels) {
                $productLevelThreeTag = $post->standardTags()->where('id', '<>', $standardTag->id)->whereHas('levelTHree')->first();
                if ($productLevelThreeTag) {
                    ...
                    if ($exsistInlevelFour) {
                        $post->standardTags()->syncWithOutDetaching($standardTag->id);
                    }
                }
            } else {
                $post->standardTags()->syncWithOutDetaching($standardTag->id);
    ...
    public function assignTags($moduleId, $uuid)
    {
        try {
            ...
            $post = Product::whereUuid($uuid)->firstOrFail();
            DB::beginTransaction();
            $tags = $this->handleTags($moduleId, request()->input('tags'), $post);
            $categoryTags = collect(json_decode(request()->input('categoryTags')))->pluck('id')->toArray();
            $ignoredTagIds = $this->productIgnoredTags(request()->ignoredTags, $post);
            $productCategoryTags = $post->tags()->asTag()->where(function ($query) {
                $query->where('is_category', true)->whereHas('standardTags_', function ($query) {
                    $query->where('priority', 1);
                });
            })->get()->reject(function ($record) {
            ...
            $post->tags()->sync($orphanTags);
            $standardTagIds =  $post->standardTags()->whereNotIn('type', ['module', 'attribute'])->wherePriority(4)->pluck('id')->toArray();
            ...
            if (request()->input('removeStandardTags')) {
                ...
                $post->standardTags()->detach(request()->input('removeStandardTags'));
            }

            $removeTags = array_diff($standardTagIds, $standardTags);
            $post->standardTags()->detach($removeTags);

            $this->storeStandardTags($post, $standardTags);

            $post->tags()->detach($ignoredTagIds['ignoredOrphanTagIds']);
            $post->standardTags()->detach($ignoredTagIds['ignoredStandardTagIds']);
            ProductTagsLevelManager::checkProductTagsLevel($post);
            ProductTagsLevelManager::priorityOneTags($post, $removeCategoryTags, 'product_tag');
            ProductTagsLevelManager::priorityFour($post, $tags['orphanTags']);
            ProductTagsLevelManager::priorityTwoTags($post);
            DB::commit();
            flash('Post tags updated successfully.', 'success');
            return \back();
    ...
    // get and store ignored tags of post

<template>
    <Head title="Post Images" />
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Post Images`" :path="`Products - ${limit(post?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <ProductSidebar :post="post" :width="'w-lg-225px'" />
            <div v-if="checkUserPermissions('add_product_images')"
                ...
                <div class="row">
                    <div class="col-lg-3 p-4 ps-8 fv-row fv-plugins-icon-container">
                        <Label for="mainImage" value="Main Image" />
                        <div class="fv-row mb-4">
                            <div class="image-input image-input-outline image-input-empty" data-kt-image-input="true">
                                <div class="image-input-wrapper w-125px h-125px"
                                    :style="{ 'background-image': 'url(' + getImage(url, isSaved, 'product', post.main_image ? post.main_image.is_external : 0) + ')' }">
                                </div>
                    ...
                    <div class="p-4 pe-8 col-lg-9">
                        <Label for="otherImages" value="Other Images" />
...
export default {
    ...
    methods: {
        ...
        submit() {
            ...
            this.$inertia.post(route("posts.dashboard.posts.media.update", [this.getSelectedModuleValue(), this.post.uuid, this.post.main_image ? this.post.main_image.id : 0]), this.form, {
                errorBag: "admin",
                preserveScroll: true,
                onError: (data) => {
    ...
    mounted() {
        ...
        var submissionUrl = route('posts.dashboard.posts.media.store', [moduleId, this.post.uuid])
        $("#product").dropzone({
            url: submissionUrl,
            parallelUploads: 6,
            ...
            removedfile: function (file) {
                if (file.id) {
                    window.swal.fire({
                        ...
                        if (result.value) {
                            showWaitDialog()
                            window.axios.delete(route('posts.dashboard.posts.media.destroy', [moduleId, productMedia.uuid, file.id]))
                                .then((response) => {
                                    hideWaitDialog()

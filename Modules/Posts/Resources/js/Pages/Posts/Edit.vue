<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Edit Post`" :path="`Posts - ${limit(post?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <ProductSidebar :post="post" :width="'w-lg-820px'" />
                            ...
                            <div class="row my-5">
                                <div class="col-lg-12 py-2">
                                    ...
                                    <Input id="name" type="text"
                                        ...
                                        autofocus autocomplete="name" placeholder="Enter post title" />
...
export default {
    props: ['post', 'levelTwoTags', 'productLevelTwoTag', 'mediaSizes'],
    ...
    methods: {
        submit() {
            this.form.put(route('posts.dashboard.posts.update', [this.getSelectedModuleValue(), this.post.uuid]), {
                errorBag: "post",
                preserveScroll: true,
            });
        },
        getLevelThreeTags(tag, level) {
            ...
            setTimeout(() => {
                ...
                // selected value
                window.axios.get(route('posts.dashboard.posts.tags', [this.getSelectedModuleValue(), tag, level]), {
                    params: {
                        post: this.post.id
                    }
                })
    ...
    mounted() {
        this.form = this.$inertia.form({
            ...
            is_commentable: this?.post?.is_commentable ? true : false,
            ...
            is_featured: this?.post?.is_featured ? true : false
        });

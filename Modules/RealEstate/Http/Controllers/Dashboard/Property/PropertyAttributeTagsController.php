...
class PropertyAttributeTagsController extends Controller
{
    /**
    ...
    public function index($moduleId, $uuid)
    {
        try {
            ...
            return Inertia::render('RealEstate::Property/Settings/AttributeTags', [
    ...
    public function assignTags($moduleId, $uuid)
    {
        try {
            ...
            // to add attribute slug in array
            $attributeSlug = [];
            foreach ($standardTags as $standardTag) {
                if (in_array($standardTag?->attribute[0]?->slug, $attributeSlug) && !$standardTag->pivot) {
                    DB::rollBack();
                    flash('You cannot add more than one tag of ' . $standardTag?->attribute[0]?->name . ' attribute', 'danger');
                    return \back();
                }
                $attributeSlug[] = $standardTag?->attribute[0]?->slug;
            ...
            DB::commit();
            SyncMyCategoryProducts::syncProductToCategory($moduleId, $product, true);
            flash('Property attribute tags updated successfully.', 'success');
            return \back();

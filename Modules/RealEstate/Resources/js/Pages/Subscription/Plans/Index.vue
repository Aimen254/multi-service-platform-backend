    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <Link :href="route('real-estate.dashboard.subscription.plan.create', [getSelectedModuleValue()])"
                    v-if="checkUserPermissions('add_subscription_plan')" class="btn btn-sm btn-primary">
                        ...
                        <tbody v-if="Subscriptions?.length > 0">
                            <template v-for="(subscription, index) in Subscriptions" :key="index">
                                <tr>
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="d-flex justify-content-start flex-column"><span
                                                    class="text-dark fw-bolder text-hover-primary mb-1 fs-6">{{
                                                        ellipsis(subscription.product.name) }}</span><span
                                                    class="text-muted d-block fs-6"><small class="capitalize">Type :
                                                        {{ subscription.product.type }}</small></span>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="badge badge-light-primary text-capitalize"> ${{
                                            subscription.price.unit_amount / 100 }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <edit-section permission="edit_subscription_plan"
                                                :url="route('real-estate.dashboard.subscription.plan.edit', [getSelectedModuleValue(), subscription.product.id])"
                                                :iconType="'link'" />
                                            <delete-section permission="delete_subscription_plan"
                                                :url="route('real-estate.dashboard.subscription.plan.destroy', [getSelectedModuleValue(), subscription.product.id])" 

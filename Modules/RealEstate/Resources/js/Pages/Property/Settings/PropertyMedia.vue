<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Images`" :path="`Properties - ${limit(product?.name)}`" />
        </template>
    ...
    methods : {
        ...
        submit() {
            ...
            this.$inertia.post(route("real-estate.dashboard.properties.media.update", [this.getSelectedModuleValue(), this.product.uuid, this.product.main_image ? this.product.main_image.id : 0]), this.form, {
                errorBag: "admin",
                preserveScroll: true,
                onError: (data) => {
    ...
    mounted(){
        ...
        var submissionUrl = route('real-estate.dashboard.properties.media.store', [moduleId, this.product.uuid])
        $("#product").dropzone({
            url: submissionUrl,
            parallelUploads:6,
            ...
            removedfile: function (file) {
                if (file.id) {
                    window.swal.fire({
                        ...
                        if (result.value) {
                            showWaitDialog()
                            window.axios.delete(route('real-estate.dashboard.properties.media.destroy', [moduleId, productMedia.uuid, file.id]))
                                .then((response) => {
                                    hideWaitDialog()

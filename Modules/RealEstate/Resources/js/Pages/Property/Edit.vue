<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Edit Property`" :path="`Properties - ${limit(form?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <ProductSidebar :product="product" :width="'w-lg-720px'" />
                            ...
                            <div class="row">
                                <div class="col-lg-6 py-2">
                                    ...
                                    <Input id="name" type="text"
                                        ...
                                        autofocus autocomplete="name" placeholder="Enter Title" />
                                ...
                                <div class="col-lg-6 py-2">
                                    <Label for="type" class="required" value="Type" />
                                    <select v-model="form.type" class="form-select form-select-solid" placeholder="Select Type"
                                        :class="{ 'is-invalid border border-danger': form.errors.type }">
                                ...
                                <div class="col-lg-6 py-2">
                                    <Label for="agents" class="required" value="Agents" />
                                    <select id="agents"
                                        class="select-agent form-control-md text-capitalize form-control-solid"
                                        v-model="form.user_id"
                                        :class="{ 'is-invalid border border-danger': form.errors.user_id }"></select>
                                    <error :message="form.errors.user_id"></error>
                                </div>
                                <div class="col-lg-6 py-2">
...
export default {
    props: ['product', 'levelTwoTags', 'productLevelTwoTag', 'brokers'],
    ...
    methods: {
        submit() {
            this.form.put(route('real-estate.dashboard.broker.properties.update', [this.getSelectedModuleValue(), this.getSelectedBusinessValue(), this.product.uuid]), {
                errorBag: "product",
                preserveScroll: true,
            });
        },
        ...
        getLevelThreeTags(tag, level) {
            ...
            setTimeout(() => {
                var select2 = $(".select").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("real-estate.dashboard.broker.properties.tags", parameters),
                        dataType: "json",
                        delay: 250,
                ...
                // selected value
                window.axios.get(route('real-estate.dashboard.broker.properties.tags', [this.getSelectedModuleValue(), this.getSelectedBusinessValue(), tag, level]), {
                    params: {
        ...
        getAgents() {
            ...
            setTimeout(() => {
                var selectAgent = $(".select-agent").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("real-estate.dashboard.broker.properties.agents", parameters),
                        dataType: "json",
                        delay: 250,
                        data: function (params) {
                            return {
                        ...
                        processResults: function (data, params) {
                            params.page = params.page || 1;
                            return {
                                results: data.agents.data.map(function (item) {
                                    return item
                                }),
                                pagination: {
                                    more: params.page * 50 < data.agents.total,
                                },
                            };
                        },
                        cache: true,
                ...
                if (vm.product?.user) {
                    var agentOption = new Option(`${vm.product?.user?.first_name} ${vm.product?.user?.last_name}`, vm.product.id, true, true);
                    selectAgent.append(agentOption).trigger('change');
                }
                //   get selected value
                $(".select-agent").on("select2:select", function (e) {
                    vm.form.user_id = e.params.data.id
                });
            }, 0);
        }
    },
    ...
    mounted() {
        ...
        this.form = this.$inertia.form({
            ...
            is_commentable: this?.product?.is_commentable ? true : false,

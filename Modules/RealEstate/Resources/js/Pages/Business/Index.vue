<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`All Brokers`" :path="`Brokers`" />
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                ...
                <Link :href="route('real-estate.dashboard.brokers.create', getSelectedModuleValue())"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <!-- <span class="badge fs-7 fw-bold text-capitalize"
                                            :class="{ 'badge-light-success': business.status == 'active', 'badge-light-danger': business.status === 'inactive' || business.status === 'rejected' }">{{
                                                business.status }}
                                        </span> -->
                                        <select v-model="business.status" @change.prevent="changeStatus($event, business)" class="form-select form-select-solid text-capitalize w-100">
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            ...
                                            <delete-section permission="delete_business"
                                                :url="route('real-estate.dashboard.brokers.destroy', [getSelectedModuleValue(), business.uuid])" 
...
export default {
    ...
    props: ['businessList', 'searchedKeyword', 'orderBy', 'status', 'rating','statusOptions'],
    data() {
        return {
            businesses: null,
            StatusOptions: '',
    ...
    this.swal.fire({
        ...
        if (result.value) {
            showWaitDialog();
            this.$inertia.visit(route('real-estate.dashboard.broker.status', [this.getSelectedModuleValue(), business.id]), {
                data: {status: event.target.value}, // Include status as a parameter

<template>
    ...
    <Head title="Brokerage Requests" />
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`All Brokerage Requests`" :path="`Brokerage`" />
        ...
        <div class="card">
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <RealEstateSearchInput :callType="type" :searchedKeyword="searchedKeyword"
                    ...
                    <table class="table table-row-dashed align-middle gs-0 gy-4">
                        <thead>
                            <tr class="fw-bolder border-0 text-muted bg-light">
                                <th class="ps-4 w-25 rounded-start">Broker Name</th>
                                <th class="min-w-120px">Broker Owner</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span v-if="business.business_owner"
                                            ...
                                            {{ business.business_owner.first_name }} {{
                                            business.business_owner.last_name }}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge fs-7 fw-bold text-capitalize"
                                            :class="{ 'badge-light-success': business.status == 'active', 'badge-light-danger': business.status === 'inactive' || business.status === 'waiting for approval' }">{{
                                                business.status }}
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <approve-section permission="approve_business"
                                                :url="route('real-estate.dashboard.broker.active.status', [getSelectedModuleValue(), business.id])" />
                                            <reject-section iconType="modal" permission="approve_business"
                                                @click="openModal(business)" />
                                            <view-section permission="delete_business"
                                                :url="route('real-estate.dashboard.brokers.show', [getSelectedModuleValue(), business.uuid])" />
                                        </div>
                                    </td>
                                </tr>
                            </template>
    ...
    <business-reject-modal></business-reject-modal>
</template>
...
import RejectSection from '@/Components/RejectSection.vue'
...
import BusinessRejectModal from "./BusinessRejectModal.vue"
...
export default {
    ...
    data() {
        return {
            ...
            type: 'business',
            logoUrl: '',
            urlForFilter: 'real-estate.dashboard.brokers.index',
            filterForm: {
    ...
    methods: {
        changeStatus(business) {
            ...
            this.swal.fire({
                ...
                if (result.value) {
                    console.log();
                    showWaitDialog()
                    this.$inertia.visit(route('real-estate.dashboard.broker.active.status', [this.getSelectedModuleValue(), business.id]), {
        ...
        openModal(business) {
            this.emitter.emit("module-reject-modal", {
                business: business,
...
</script>./BusinessRejectModal.vue

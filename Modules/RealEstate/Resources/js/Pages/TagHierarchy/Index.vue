                            ...
                            <div class="card-body py-3">
                                <div class="d-flex gap-5">
                                    <div v-if="level > 2">
                                        <h3 class="card-title align-items-start flex-column">
                                            <span class="card-label fw-bolder fs-3 mb-1">
                                                ...
                                                }" class="business" :placeholder="`Select level 2 tag`"
                                                    v-model="levelTwoTag" :options="levelTwoTags"
                                    ...
                                    <div v-if="level > 3" style="width:220px">
                                        <h3 class="card-title align-items-start flex-column">
                                            <span class="card-label fw-bolder fs-3 mb-1">
                                                <select class="select" v-model="levelThreeTag"></select>
                                            </span>
                                        </h3>
                                ...
                                <div class="col-lg-12 fv-row fv-plugins-icon-container">
                                    ...
                                    <div class="fv-row mb-4">
                                        <VueTagsInput style="height: "
                                            class="tags-input form-control form-control-md form-control-solid w-full bg-transparent"
                                            v-model="tag" :placeholder="`Level ${level} tags`" :tags="form['L' + level]"
                                            :autocomplete-items="all_tags" :add-only-from-autocomplete="true"
                                            @before-deleting-tag="deleteTag" @tags-changed="updateTags"
                                            @keyup="getAutoComplete($event)" />
                                        <error :message="form.errors['L' + level]"></error>
                                        <div class="mt-4" v-if="meta && meta.data.length > 0">
                                            <pagination @pageNo="pagination" :meta="meta" :section="'hierarchy'"
                                                :keyword="searchedKeyword" :selectedFilters="filterForm" :callType="type" />
                                        </div>
                                    </div>
                                </div>
...
export default {
    ...
    methods: {
        submit() {
            this.form.post(
                route("real-estate.dashboard.tag-hierarchies.store", [
        ...
        async getTags(e, dropdown, page = null) {
            ...
            if ((this.level == 3 && dropdown == "levelTwo") || (this.level == 4 && dropdown == "levelThree")) {
                ...
                axios
                    .get(route("real-estate.dashboard.tag-hierarchies.getTagWithLevel", parameters), {
                        params: {
                            page: page ? page : 1
                        },
                    })
        ...
        deleteTag(e) {
            axios({
                method: "get",
                url: route("real-estate.dashboard.tag-hierarchies.show", [
                    this.getSelectedModuleValue(),
                    this.level,
                    e.tag.id,
                ]),
        ...
        searchTags() {
            ...
            this.debounce = setTimeout(() => {
                window.axios
                    .get(
                        route(
                            "real-estate.dashboard.search.standardTags",
                            this.getSelectedModuleValue()
                        ),
                        {
                            params: {
        ...
        levelThreeDropdown() {
            ...
            setTimeout(() => {
                $(".select").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("real-estate.dashboard.tag-hierarchies.getTagWithLevel", parameters),
                        dataType: "json",
                        delay: 250,

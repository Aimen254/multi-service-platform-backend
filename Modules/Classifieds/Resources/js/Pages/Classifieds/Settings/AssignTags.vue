<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Assign Tags`" :path="`Classifieds - ${limit(classified?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <ClassifiedSidebar :classified="classified" style="width: 25%!important;" />
                    ...
                    <div class="card card-flush py-4">
                        <div class="card-body py-3 row">
                            ...
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Tags" />
                                <TagComponent placeholder="Tags" :uuid="classified.uuid" :assignedTags="productTags" :onlyFromAutoComplete="true" :openOnClick="false" :tooltipContent="'Mapped Tags Information.'"/>
                                <error :message="form.errors.tags" />
                            </div>
                            <!-- Extra -->
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Extra Tags" />
                                <TagComponent placeholder="Extra Tags" :uuid="classified.uuid" :assignedTags="extraTag" :autocomplete="filteredAllTags" :onlyFromAutoComplete="false" :openOnClick="false"  :tooltipContent="'Unmapped Tags Information.'"/>
                                <error :message="form.errors.tags" />
                            </div>
                            <!-- Brand tags -->
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Brand tags" />
                                <TagComponent placeholder="Brand tags" :uuid="classified.uuid" :assignedTags="allproductBrandTags" type="brand" :autocomplete="allBrandTags" :onlyFromAutoComplete="true" openOnClick="true"  :tooltipContent="'Ignored Inappropriate Tags.'"/>
                                <error :message="form.errors.tags" />
                            </div>
                            <!-- Ignore tags -->
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Ignored Tags" />
                                <TagComponent placeholder="Ignored Tags" :uuid="classified.uuid" :assignedTags="ignoredTags" :autocomplete="allIgnoredAutocomplete" :onlyFromAutoComplete="false" :openOnClick="true"/>
                                <error :message="form.errors.tags" />
                            </div>
...
export default {
    props: ['classified', 'allTags', 'allBrandTags', 'allproductBrandTags', 'allproductTags', 'extraTags', 'allIgnoredAutocomplete', 'productIgnoredTags'],
    ...
    methods: {
        submit () {
            ...
            this.form.get(route("classifieds.dashboard.classifieds.tags.assign", [this
                .getSelectedModuleValue(), this.classified.uuid
            ]));
        },
        allTagsFn() {

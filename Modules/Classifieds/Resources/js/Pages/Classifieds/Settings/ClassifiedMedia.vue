<template>
    <Head title="Classified Images" />
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Classified Images`" :path="`Products - ${limit(classified?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <ClassifiedSidebar :classified="classified" :width="'w-lg-225px'"/>
                        ...
                        <div class="fv-row mb-4">
                            <div class="image-input image-input-outline image-input-empty" data-kt-image-input="true">
                                <div class="image-input-wrapper w-125px h-125px"
                                    :style="{ 'background-image': 'url(' + getImage(url, isSaved, 'classifieds', classified.main_image ? classified.main_image.is_external : 0) + ')' }"></div>
    ...
    methods : {
        ...
        submit() {
            ...
            this.$inertia.post(route("classifieds.dashboard.classifieds.media.update", [this.getSelectedModuleValue(), this.classified.uuid, this.classified.main_image ? this.classified.main_image.id : 0]), this.form, {
                errorBag: "admin",
                preserveScroll: true,
                onError: (data) => {
    ...
    mounted(){
        var productMedia = this.classified
        this.form = useForm({
            image: "",
        });
        ...
        var submissionUrl = route('classifieds.dashboard.classifieds.media.store', [moduleId, this.classified.uuid])
        $("#product").dropzone({
            url: submissionUrl,
            parallelUploads:6,
            ...
            removedfile: function (file) {
                if (file.id) {
                    window.swal.fire({
                        ...
                        if (result.value) {
                            showWaitDialog()
                            window.axios.delete(route('classifieds.dashboard.classifieds.media.destroy', [moduleId, productMedia.uuid, file.id]))
                                .then((response) => {
                                    hideWaitDialog()

<template>
    <Head title="Event Images" />
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Event Images`" :path="`Products - ${limit(event?.name, 36)}`" />
        </template>
                        ...
                        <div class="fv-row mb-4">
                            <div class="image-input image-input-outline image-input-empty" data-kt-image-input="true">
                                <div class="image-input-wrapper w-125px h-125px"
                                    :style="{ 'background-image': 'url(' + getImage(url, isSaved, 'events', event.main_image ? event.main_image.is_external : 0) + ')' }"></div>
    ...
    methods : {
        ...
        submit() {
            ...
            this.$inertia.post(route("events.dashboard.events.media.update", [this.getSelectedModuleValue(), this.event.uuid, this.event.main_image ? this.event.main_image.id : 0]), this.form, {
                errorBag: "admin",
                preserveScroll: true,
                onError: (data) => {
    ...
    mounted(){
        ...
        var submissionUrl = route('events.dashboard.events.media.store', [moduleId, this.event.uuid])
            ...
            removedfile: function (file) {
                if (file.id) {
                    window.swal.fire({
                        ...
                        if (result.value) {
                            showWaitDialog()
                            window.axios.delete(route('events.dashboard.events.media.destroy', [moduleId, productMedia.uuid, file.id]))
                                .then((response) => {
                                    hideWaitDialog()

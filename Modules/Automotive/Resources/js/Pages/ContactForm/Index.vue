<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Contact Forms`" :path="`Contact Forms`" />
        </template>
        <div class="card">
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <AutomotiveSearchInput :module="module" :callType="type" :searchedKeyword="searchedKeyword"/>
                </h3>
            </div>
                    ...
                    <table class="table table-row-dashed align-middle gs-0 gy-4">
                        <thead>
                            <tr class="fw-bolder border-0 text-muted bg-light">
                                <th class="ps-4 min-w-120px rounded-start">Vehicle</th>
                        ...
                        <tbody v-if="contactForms && contactForms.data.length > 0">
                            <template v-for="form in contactForms.data" :key="form.id">
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <view-section permission="view_contact_form" :url="route('automotive.dashboard.dealership.contact-form.show', [getSelectedModuleValue(), getSelectedBusinessValue(), form.id])"/>
                                            ...
                                            <delete-section permission="delete_contact_form"
                                                :url="route('automotive.dashboard.dealership.contact-form.destroy', [getSelectedModuleValue(), getSelectedBusinessValue(), form.id])" 

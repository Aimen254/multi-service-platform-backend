<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Vehicles`" :path="`Vehicles`" />
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <div class="me-4">
                    ...
                    <Filter @removeSelectedTags="getTags()" :filterData="filterForm" :callType="type" :business="business" :url="urlForFilter"
                        ...
                        <div class="mb-5">
                            ...
                            <div>
                                <Datepicker
                                    v-model="filterForm.year" year-picker></Datepicker>
                            </div>
                        </div>
                ...
                <Link v-if="checkUserPermissions('add_products')"
                    :href="route('automotive.dashboard.dealership.vehicles.create', [this.getSelectedModuleValue(), getSelectedBusinessValue()])"
        ...
        <div class="card">
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <AutomotiveSearchInput :module="module" :callType="type" :searchedKeyword="searchedKeyword" :filterFormData="filterForm" :year="filterForm.year"/>
                </h3>
            </div>
                                ...
                                <tr
                                   ...
                                   <td>
                                        ...
                                        <select v-model="vehicle.status" v-else
                                            @change.prevent="changeStatus($event, vehicle.uuid)" class="form-select form-select-solid text-capitalize w-100">
                                            <option value="active">Active</option>
                                            <option value="inactive">InActive</option>
                                            <option value="pending">Pending</option>
                                            <option value="sold">Sold</option>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <edit-section iconType="link" permission="edit_products"
                                                :url="route('automotive.dashboard.dealership.vehicles.edit', [getSelectedModuleValue(), getSelectedBusinessValue(), vehicle.uuid])" />
                                            <delete-section permission="delete_products"
                                                :url="route('automotive.dashboard.dealership.vehicles.destroy', [getSelectedModuleValue(), getSelectedBusinessValue(), vehicle.uuid])"
            ...
            <pagination :meta="vehicles" :keyword="searchedKeyword" :selectedFilters="filterForm" :callType="type" :year="filterForm.year" />
        </div>
    </AuthenticatedLayout>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            type: 'vehicles',
            urlForFilter: 'automotive.dashboard.dealership.vehicles.index',
            filterForm: {
    ...
    methods: {
        ...
        changeStatus(event, uuid) {
            ...
            this.swal.fire({
                ...
                if (result.value) {
                    showWaitDialog()
                    this.form.post(route('automotive.dashboard.dealership.vehicle.status', [this.getSelectedModuleValue(), this.getSelectedBusinessValue(), uuid]), {
         ...
         getTags() {
            ...
            setTimeout(() => {
                $(".select").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("automotive.dashboard.dealership.search.vehicle.tags", parameters),
                        dataType: "json",
                        delay: 250,

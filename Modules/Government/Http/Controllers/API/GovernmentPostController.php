...
class GovernmentPostController extends Controller
{
    ...
    public function index($request, $moduleId)
    {
        ...
        if (request()->filled('is_featured')) {
            ...
            $products = Product::when(request()->input('user_id'), function($query) {
                $query->when(request()->input('role') == 'government_staff', function ($query) {
                    $query->where('user_id', request()->input('user_id'));
                }, function ($query) {
                    $query->whereRelation('business', 'owner_id', request()->input('user_id'));
                });
            })->when(!request()->input('role') || request()->input('role') != 'government_staff', function ($query) use ($moduleId) {
                $query->whereStandardTag($moduleId);
            })->whereRelation('standardTags', 'id', $moduleId)->whereRelation('business', 'status', 'active')->withCount(['comments', 'wishList', 'views', 'likes'])
                ->where(function ($query) {
                    ...
                    if (request()->input('business_id') || request()->input('business_slug')) {
                        $query->whereHas('business', function ($query) {
                            $query->where('id', request()->input('business_id'))->orWhere('slug', request()->input('business_slug'));
                        });
                    }

...
use Modules\Government\Http\Requests\TagHierarchyRequest;
...
class HierarchiesController extends Controller
{
    ...
    public function index(Request $request, $module_id, $level)
    {
        ...
        $assignedTags = StandardTag::asTag()->where('type', 'product')
        ...
        return Inertia::render('Government::TagHierarchy/Index', [
    ...
    public function getTagWithLevel($module_id, $levelTwo, $levelThree = null)
    {
        ...
        $levelTags = StandardTag::asTag()->where('type', 'product')->active()
            ->where('name', 'like', '%' . request()->input('keyword') . '%')->where(function($query) use ($module_id, $levelTwo, $levelThree, $relationName) {
                $query->whereHas('tagHierarchies', function ($query) use ($module_id, $levelTwo, $levelThree) {
                    $query->where('L1', $module_id)->where('L2', $levelTwo)->when($levelThree, function ($query) use ($levelThree) {

        ...
        <template #breadcrumbs>
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <div class="me-4">
                    ...
                    <Filter @removeSelectedTags="getTags()" :filterData="filterForm" :callType="type" :business="business"
                        :url="urlForFilter" :newKeyword="searchedKeyword">
                        <div class="mb-5">
                ...
                <Link v-if="checkUserPermissions('add_products')"
                    :href="route('government.dashboard.department.posts.create', [getSelectedModuleValue(), getSelectedBusinessValue()])"
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="d-flex justify-content-start flex-column mx-2">
                                                <span class="text-dark fw-bolder text-hover-primary mb-1 fs-6"
                                                    ...
                                                    :data-bs-original-title="product.name ? product.name : removeDescriptionHtml(product.description)">
                                                    {{ ellipsis(product.name ? product.name :
                                                        removeDescriptionHtml(product.description)) }}
                                                </span>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            ...
                                            <edit-section iconType="link" permission="edit_products"
                                                :url="route('government.dashboard.department.posts.edit', [getSelectedModuleValue(), getSelectedBusinessValue(), product.uuid])" />
                                            <delete-section permission="delete_products"
                                                :url="route('government.dashboard.department.posts.destroy', [getSelectedModuleValue(), getSelectedBusinessValue(), product.uuid])" 
...
import { name } from '@shapla/vue-star-rating';
...
export default {
    ...
    data() {
        return {
            ...
            type: 'posts',
            urlForFilter: 'government.dashboard.department.posts.index',
            filterForm: {
        ...
        changeStatus(uuid) {
            this.swal.fire({
                ...
                if (result.value) {
                    ...
                    this.$inertia.visit(route('government.dashboard.department.post.status', [this.getSelectedModuleValue(), this.getSelectedBusinessValue(), uuid]), {
        ...
        getTags() {
            ...
            setTimeout(() => {
                $(".select").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("government.dashboard.department.search.post.tags", parameters),
                        dataType: "json",
                        delay: 250,

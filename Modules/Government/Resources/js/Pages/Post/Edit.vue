<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Edit Post`" :path="`Posts - ${limit(form?.name, 36)}`" />
        </template>
                        ...
                        <div class="card-body py-3 row">
                            ...
                            <div class="row" style="margin-top: 12%;">
                                ...
                                <div class="form-check pt-2">
                                    <input class="form-check-input" type="checkbox" id="featureCheck" value="" :checked="form.is_featured"
                                        v-model="form.is_featured" />
                                    <label class="form-check-label fw-bolder" for="featureCheck">
                                        Featured
                                    </label>
...
export default {
    ...
    methods: {
        submit() {
            this.form.put(route('government.dashboard.department.posts.update', [this.getSelectedModuleValue(), this.getSelectedBusinessValue(), this.product.uuid]), {
                errorBag: "product",
                preserveScroll: true,
            });
        },
        ...
        getLevelThreeTags(tag, level) {
            ...
            setTimeout(() => {
                var select2 = $(".select").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("government.dashboard.department.post.tags", parameters),
                        dataType: "json",
                        delay: 250,
                ...
                // selected value
                window.axios.get(route('government.dashboard.department.post.tags', [this.getSelectedModuleValue(), this.getSelectedBusinessValue(), tag, level]), {
                    params: {

<template>
  <Head title="Department Schedule" />
  <AuthenticatedLayout>
    <template #breadcrumbs>
      <Breadcrumbs :title="`Department Schedule`" :path="`Departments`"></Breadcrumbs>
    </template>
  ...
  <DepartmentScheduleModal :business="business" v-on:childToParent="onChildClick"></DepartmentScheduleModal>
</template>
...
import DepartmentScheduleModal from "./DepartmentScheduleModal.vue"
import DepartmentHolidayModal from "./DepartmentHolidayModal.vue"
...
export default {
  ...
  methods: {
      ...
      this.swal
        ...
        .then((result) => {
          if (result.value) {
            window.axios
              .delete(
                route("government.dashboard.department.departmentschedule.destroy", [
      ...
      this.swal
        ...
        .then((result) => {
          if (result.value) {
            window.axios
              .delete(
                route("government.dashboard.department.departmentholidays.destroy", [
                  this.getSelectedModuleValue(),
                  id,
                  this.business.uuid,
                ])
    ...
    updateBusinessSchedule(businessUuid, scheduleId) {
      window.axios
        .post(
          route("government.dashboard.department.departmentschedule.status", [
            this.getSelectedModuleValue(),
            businessUuid,
            scheduleId,
            businessUuid,
    ...
    getSchedule(businessId, ScheduleId, ScheduleName) {
      ...
      window.axios
        .get(
          route("government.dashboard.department.departmentschedule.schedule", [
            this.getSelectedModuleValue(),
            businessId,
            ScheduleId,
          ])
    ...
    getHolidays(businessId) {
      window.axios
        .get(
          route("government.dashboard.department.departmentholidays.show", [
            this.getSelectedModuleValue(),
            businessId,
            businessId,
          ])

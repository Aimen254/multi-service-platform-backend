<template>
    <Head title="Blog Images" />
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Blog Images`" :path="`Products - ${limit(blog?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <BlogSidebar :blog="blog" :width="'w-lg-225px'"/>
                        ...
                        <div class="fv-row mb-4">
                            <div class="image-input image-input-outline image-input-empty" data-kt-image-input="true">
                                <div class="image-input-wrapper w-125px h-125px"
                                    :style="{ 'background-image': 'url(' + getImage(url, isSaved, 'blogs', blog.main_image ? blog.main_image.is_external : 0) + ')' }"></div>
    ...
    methods : {
        ...
        submit() {
            ...
            this.$inertia.post(route("blogs.dashboard.blogs.media.update", [this.getSelectedModuleValue(), this.blog.uuid, this.blog.main_image ? this.blog.main_image.id : 0]), this.form, {
                errorBag: "admin",
                preserveScroll: true,
                onError: (data) => {
    ...
    mounted(){
        ...
        var submissionUrl = route('blogs.dashboard.blogs.media.store', [moduleId, this.blog.uuid])
        $("#product").dropzone({
            url: submissionUrl,
            parallelUploads:6,
            ...
            removedfile: function (file) {
                if (file.id) {
                    window.swal.fire({
                        ...
                        if (result.value) {
                            showWaitDialog()
                            window.axios.delete(route('blogs.dashboard.blogs.media.destroy', [moduleId, productMedia.uuid, file.id]))
                                .then((response) => {
                                    hideWaitDialog()

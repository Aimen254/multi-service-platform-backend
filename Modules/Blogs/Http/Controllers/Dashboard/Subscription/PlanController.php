...
namespace Modules\Blogs\Http\Controllers\Dashboard\Subscription;
...
class PlanController extends Controller
{
    ...
    public function index($moduleId)
    {
        ...
        return Inertia::render('Blogs::Subscription/Plans/Index', [
    ...
    {
        return Inertia::render('Blogs::Subscription/Plans/Edit', []);
    }
    ...
    public function update(SubscriptionPlanRequest $request, $moduleId, $id)
    {
        try {
            ...
            return \redirect()->route('blogs.dashboard.subscription.plan.index', $moduleId);
        } catch (\Stripe\Exception\CardException $e) {
            flash($e->getMessage(), 'danger');
            return \redirect()->back();
        } catch (\Stripe\Exception\InvalidRequestException $e) {
    ...
    public function destroy($moduleId, $id, Request $request)
    {
        try {
            ...
            if ($currentCount > 1) {
                ...
                return Redirect::route('blogs.dashboard.subscription.plan.index', [$moduleId, 'page' => $previousPage]);
            }
        } catch (\Stripe\Exception\CardException $e) {
            flash($e->getMessage(), 'danger');
            return \redirect()->back();

...
class BlogTagController extends Controller
{
    ...
    public function index($moduleId, $uuid)
    {
        try {
            ...
            // Getting orphan tags
            $orphanTags = $blog->tags()->asTag()->where(function ($query) {
            ...
            $extraOrphanTags = $blog->tags()->asTag()->whereDoesntHave('standardTags_')->get();
            // Getting extra standard tags
            $extraStandardTags = $blog->standardTags()->asTag()->wherePriority(4)->whereHas('tags_')->get();
            // Getting tags
            $extraTags = Arr::collapse([$extraOrphanTags, $extraStandardTags]);
            // Getting ignored orphan tags
            $ignoredOrphanTags = $blog->tags()->asTag()->whereHas('standardTags_', function ($query) {
                $query->where('priority', '<>', 1);
            })->orWhereDoesntHave('standardTags_')->where('product_id', $blog->id)->get()->reject(function ($record) {
                $tag = $record->whereHas('standardTags_', function ($query) use ($record) {
                    $query->where('priority', '<>', 1)->where('slug', $record->slug);
                })->first();
                return $tag ? true : false;
            ...
            $ignoredTags = $blog->ignoredTags()->first();
            $productIgnoredTags = [];
            if ($ignoredTags) {
            ...
            $ignoredStandardTags = $blog->standardTags()->asTag()->where('type', '<>', 'module')->where('priority', '<>', 1)->get();

            return inertia('Blogs::Blogs/Settings/AssignTags', [
                'blog' => $blog,

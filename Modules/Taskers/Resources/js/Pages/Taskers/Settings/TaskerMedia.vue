<template>
    <Head title="Tasker Images" />
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Tasker Images`" :path="`Products - ${limit(tasker?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <TaskerSidebar :tasker="tasker" :width="'w-lg-225px'" />
            <div v-if="checkUserPermissions('add_product_images')"
                class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10 bg-white" id="dropzone_div">
                <div class="row">
                        ...
                        <div class="fv-row mb-4">
                            <div class="image-input image-input-outline image-input-empty" data-kt-image-input="true">
                                <div class="image-input-wrapper w-125px h-125px"
                                    :style="{ 'background-image': 'url(' + getImage(url, isSaved, 'taskers', tasker.main_image ? tasker.main_image.is_external : 0) + ')' }">
                                </div>
                    ...
                    <div class="p-4 pe-8 col-lg-9">
                        ...
                        <div id="product" class="dropzone">
                            ...
                            <h3 class="fs-5 fw-bolder text-gray-900 mt-3" id="text_message">Drop files here or click to upload.</h3>
                        </div>
...
export default {
    ...
    methods: {
        ...
        submit() {
            ...
            this.$inertia.post(route("taskers.dashboard.taskers.media.update", [this.getSelectedModuleValue(), this.tasker.uuid, this.tasker.main_image ? this.tasker.main_image.id : 0]), this.form, {
                errorBag: "admin",
                preserveScroll: true,
                onError: (data) => {
    ...
    mounted() {
        ...
        var submissionUrl = route('taskers.dashboard.taskers.media.store', [moduleId, this.tasker.uuid])
        $("#product").dropzone({
            url: submissionUrl,
            parallelUploads: 6,
            ...
            error: function (xhr, status) {
                ...
                $('#product').after('<div class="text-danger mt-2" id="error">' + status.message + '</div>');
                $('#product').find('div.dz-processing').remove();
            ...
            removedfile: function (file) {
                if (file.id) {
                    window.swal.fire({
                        ...
                        if (result.value) {
                            showWaitDialog()
                            window.axios.delete(route('taskers.dashboard.taskers.media.destroy', [moduleId, productMedia.uuid, file.id]))
                                .then((response) => {
                                    hideWaitDialog()

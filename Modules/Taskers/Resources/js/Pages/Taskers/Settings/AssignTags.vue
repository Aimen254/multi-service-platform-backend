<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Assign Tags`" :path="`Taskers - ${limit(tasker?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <TaskerSidebar :tasker="tasker" style="width: 25%!important;" />
                    ...
                    <div class="card card-flush py-4">
                        <div class="card-body py-3 row">
                            ...
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Tags" />
                                <TagComponent placeholder="Tags" :uuid="tasker.uuid" :assignedTags="productTags" :onlyFromAutoComplete="true" :openOnClick="false" :tooltipContent="'Mapped Tags Information.'"/>
                                <error :message="form.errors.tags" />
                            </div>
                            <!-- Extra -->
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Extra Tags" />
                                <TagComponent placeholder="Extra Tags" :uuid="tasker.uuid" :assignedTags="extraTag" :autocomplete="filteredAllTags" :onlyFromAutoComplete="false" :openOnClick="false" :tooltipContent="'Unmapped Tags Information.'"/>
                                <error :message="form.errors.tags" />
                            </div>
                            <!-- Brand tags -->
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Brand tags" />
                                <TagComponent placeholder="Brand tags" :uuid="tasker.uuid" :assignedTags="allproductBrandTags" type="brand" :autocomplete="allBrandTags" :onlyFromAutoComplete="true" openOnClick="true"  :tooltipContent="'Brand Tags.'"/>
                                <error :message="form.errors.tags" />
                            </div>
                            <!-- Ignore tags -->
                            <div class="fv-row mb-4 fv-plugins-icon-container">
                                <Label value="Ignored Tags" />
                                <TagComponent placeholder="Ignored Tags" :uuid="tasker.uuid" :assignedTags="ignoredTags" :autocomplete="allIgnoredAutocomplete" :onlyFromAutoComplete="false" :openOnClick="true"  :tooltipContent="'Ignored Inappropriate Tags.'"/>
                                <error :message="form.errors.tags" />
                            </div>
...
export default {
    props: ['tasker', 'allTags', 'allBrandTags', 'allproductBrandTags', 'allproductTags', 'extraTags', 'allIgnoredAutocomplete', 'productIgnoredTags'],
    ...
    methods: {
        submit () {
            ...
            this.form.get(route("taskers.dashboard.taskers.tags.assign", [this
                .getSelectedModuleValue(), this.tasker.uuid
            ]));
        },
        allTagsFn() {

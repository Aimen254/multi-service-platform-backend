...
namespace Modules\Taskers\Http\Controllers\Dashboard\Subscription;
...
{
    public function __construct(protected StripeClient $stripeClient, private $product = [])
    {
    }
    ...
    public function index($moduleId)
    {
        ...
        return Inertia::render('Taskers::Subscription/Plans/Index', [
    ...
    {
        return Inertia::render('Taskers::Subscription/Plans/Edit', []);
    }
    ...
    public function update(SubscriptionPlanRequest $request, $moduleId, $id)
    {
        try {
            ...
            return \redirect()->route('taskers.dashboard.subscription.plan.index', $moduleId);
        } catch (\Stripe\Exception\CardException $e) {
            flash($e->getMessage(), 'danger');
    ...
    public function destroy($moduleId, $id,Request $request)
    {
        try {
            ...
            if ($currentCount > 1) {
                ...
                return Redirect::route('taskers.dashboard.subscription.plan.index', [$moduleId, 'page' => $previousPage]);
            }
        } catch (\Stripe\Exception\CardException $e) {
            flash($e->getMessage(), 'danger');
            return \redirect()->back();

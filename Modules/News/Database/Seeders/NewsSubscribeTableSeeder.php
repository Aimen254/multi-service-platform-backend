...
class NewsSubscribeTableSeeder extends Seeder
{
    ...
    {
        Model::unguard();
        $reporter01 = User::whereEmail('reporter01@interapptive.com')->with('cards')->first();
        $reporter02 = User::whereEmail('reporter02@interapptive.com')->with('cards')->first();
        $reporters = [
            $reporter01->stripe_customer_id,
            $reporter02->stripe_customer_id,
        ];
        $cards = [
            $reporter01->cards[0]->payment_method_id,
            $reporter02->cards[0]->payment_method_id,
        ];
        foreach ($reporters as $key => $cusId) {
            $module = StandardTag::where('slug', 'news')->firstOrFail();

...
class NewsTagController extends Controller
{
    ...
    public function index($moduleId, $uuid)
    {
        try {
            ...
            // Getting orphan tags
            $orphanTags = $news->tags()->asTag()->where(function ($query) {
            ...
            $extraOrphanTags = $news->tags()->asTag()->whereDoesntHave('standardTags_')->get();
            // Getting extra standard tags
            $extraStandardTags = $news->standardTags()->asTag()->wherePriority(4)->whereHas('tags_')->get();
            // Getting tags
            $extraTags = Arr::collapse([$extraOrphanTags, $extraStandardTags]);
            // Getting ignored orphan tags
            $ignoredOrphanTags = $news->tags()->asTag()->whereHas('standardTags_', function ($query) {
                $query->where('priority', '<>', 1);
            })->orWhereDoesntHave('standardTags_')->where('product_id', $news->id)->get()->reject(function ($record) {
                $tag = $record->whereHas('standardTags_', function ($query) use ($record) {
                    $query->where('priority', '<>', 1)->where('slug', $record->slug);
                })->first();
                return $tag ? true : false;
            ...
            $ignoredTags = $news->ignoredTags()->first();
            $productIgnoredTags = [];
            if ($ignoredTags) {
            ...
            $ignoredStandardTags = $news->standardTags()->asTag()->where('type', '<>', 'module')->where('priority', '<>', 1)->get();

            return inertia('News::News/Settings/AssignTags', [
                'news' => $news,
    ...
    public function assignTags($moduleId, $uuid)
    {
        try {
            ...
            if (request()->input('removeStandardTags')) {
                ...
                $news->standardTags()->detach(request()->input('removeStandardTags'));
            }
            ...
            DB::commit();
            flash('News tags updated successfully.', 'success');
            return \redirect()->back();

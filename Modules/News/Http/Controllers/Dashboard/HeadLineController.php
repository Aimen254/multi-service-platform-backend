...
namespace Modules\News\Http\Controllers\Dashboard;
...
class HeadLineController extends Controller
{
    ...
    public function index($moduleId = null)
    {
        $headLines = Product::whereHas('headline')
            ...
            ->orderByRaw("CASE 
                ...
                ELSE 2 END")
            ->paginate(5);
        return Inertia::render('News::HeadLines/Index', [
            'headLines' => $headLines
        ]);
    }
    ...
    public function create($moduleId)
    {
        ...
        if ($type) {
            ...
            $news = Product::moduleBasedProducts($moduleId)->where('user_id', '!=', null)->with(['mainImage', 'headline'])->where(function ($query) use ($tag) {
                if (request()->keyword) {
        ...
        return Inertia::render('News::HeadLines/Create', [
            'levelTwoTags' => $tags,
            'newsList' => $news?->count() > 0 ? $news : null,
            'type' => $type,
            'tag' => $tag,
    ...
    public function store(Request $request, $moduleId)
    {
        try {
            if ($request->type == 'Primary') {
                ...
                if ($existingHeadline) {
                    ...
                    return response()->json([
                        ...
                        'message' => 'News updated to ' . $request->type .  ' headline'
            ...
            return response()->json([
                'status' => JsonResponse::HTTP_OK,
                'message' => 'News added to ' . $request->type .  ' headline'
            ], JsonResponse::HTTP_OK);
        } catch (\Exception $e) {
    ...
    public function destroy($moduleId, $id, Request $request)
    {
        try {
            ...
            if ($currentCount > 1) {
                ...
                return Redirect::route('news.dashboard.headlines.index', [$moduleId, $id, 'page' => $previousPage]);
            }

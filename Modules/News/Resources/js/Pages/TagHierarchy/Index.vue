<template>
  ...
  export default {
    ...
    methods: {
      submit() {
        this.form.post(
          route("news.dashboard.tag-hierarchies.store", [
      ...
      async getTags(e, dropdown, page = null) {
        ...
        if ((this.level == 3 && dropdown == "levelTwo") || (this.level == 4 && dropdown == "levelThree")) {
          ...
          axios
            .get(route("news.dashboard.tag-hierarchies.getTagWithLevel", parameters), {
               params: {
                 page: page ? page : 1 
                },
            })
      ...
      deleteTag(e) {
        axios({
          method: "get",
          url: route("news.dashboard.tag-hierarchies.show", [
            this.getSelectedModuleValue(),
            this.level,
            e.tag.id,
          ]),
      ...
      searchTags() {
        ...
        this.debounce = setTimeout(() => {
          window.axios
            .get(
              route(
                "news.dashboard.search.standardTags",
                this.getSelectedModuleValue()
              ),
              {
                params: {
       ...
       levelThreeDropdown() {
        ...
        setTimeout(() => {
          $(".select").select2({
            ...
            // ajax request
            ajax: {
              url: route("news.dashboard.tag-hierarchies.getTagWithLevel", parameters),
              dataType: "json",
              delay: 250,

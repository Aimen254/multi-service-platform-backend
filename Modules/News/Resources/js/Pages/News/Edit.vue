<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Edit News`" :path="`News - ${limit(form?.name, 36)}`" />
        </template>
        <div class="d-flex flex-column flex-lg-row">
            <ProductSidebar :news="news" :width="'w-lg-820px'" />
                            ...
                            <div class="row mt-8">
                                <div class="col-lg-12 py-2">
                                    ...
                                    <Input id="name" type="text"
                                        ...
                                        autofocus autocomplete="name" placeholder="Enter news title" />
...
export default {
    props: ['news', 'levelTwoTags', 'productLevelTwoTag', 'mediaSizes'],
    ...
    methods: {
        submit() {
            this.form.put(route('news.dashboard.news.update', [this.getSelectedModuleValue(), this.news.uuid]), {
                errorBag: "news",
                preserveScroll: true,
            });
        },
        getLevelThreeTags(tag, level) {
            ...
            setTimeout(() => {
                var select2 = $(".select").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("news.dashboard.news.tags", parameters),
                        dataType: "json",
                        delay: 250,
                ...
                // selected value
                window.axios.get(route('news.dashboard.news.tags', [this.getSelectedModuleValue(), tag, level]), {
                    params: {
                        news: this.news.id
                    }
                })
    ...
    mounted() {
        this.form = this.$inertia.form({
            ...
            is_featured: this.news.is_featured ? true : false,
            is_commentable: this?.news?.is_commentable ? true : false,

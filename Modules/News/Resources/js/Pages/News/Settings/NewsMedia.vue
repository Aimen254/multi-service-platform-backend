<template>
<Head title="News Images" />
<AuthenticatedLayout>
    <template #breadcrumbs>
        <Breadcrumbs :title="`News Images`" :path="`News - ${limit(news?.name, 36)}`" />
    </template>
    <div class="d-flex flex-column flex-lg-row">
        <ProductSidebar :news="news" :width="'w-lg-225px'"/>
                    ...
                    <div class="fv-row mb-4">
                        <div class="image-input image-input-outline image-input-empty" data-kt-image-input="true">
                            <div class="image-input-wrapper w-125px h-125px"
                                :style="{ 'background-image': 'url(' + getImage(url, isSaved, 'news', news.main_image ? news.main_image.is_external : 0) + ')' }"></div>
...
methods : {
    ...
    submit() {
        ...
        this.$inertia.post(route("news.dashboard.news.media.update", [this.getSelectedModuleValue(), this.news.uuid, this.news.main_image ? this.news.main_image.id : 0]), this.form, {
            errorBag: "admin",
            preserveScroll: true,
            onError: (data) => {
...
mounted(){
    ...
    var submissionUrl = route('news.dashboard.news.media.store', [moduleId, this.news.uuid])
    $("#product").dropzone({
        url: submissionUrl,
        parallelUploads:6,
        ...
        removedfile: function (file) {
            if (file.id) {
                window.swal.fire({
                    ...
                    if (result.value) {
                        showWaitDialog()
                        window.axios.delete(route('news.dashboard.news.media.destroy', [moduleId, productMedia.uuid, file.id]))
                            .then((response) => {
                                hideWaitDialog()

            ...
            <form class="mb-4" v-if="form" @submit.prevent="submit">
                <div class="row fv-row mb-4 fv-plugins-icon-container">
                    <div class="col-md-6">
                        ...
                        <vue-timepicker v-model="form.open_at" format="hh:mm A" :minute-interval="15"></vue-timepicker>
                        <error v-if="error && errors.open_at" :message="errors.open_at[0]">
                        </error>
                    </div>
                    <div class="col-md-6">
                        <Label for="close_at" class="required" value="Close At" />
                        <vue-timepicker v-model="form.close_at" format="hh:mm A" :minute-interval="15"></vue-timepicker>
                ...
                <div class="text-end">
                    <Button type="submit" :class="{ 'opacity-25': loading }" :disabled="loading">
                        <span class="indicator-label" v-if="!loading"> {{ 'Save' }}</span>
...
export default {
    ...
    methods: {
        ...
        submit() {
            this.loading = true;
            if (this.type == 'post') {
                window.axios.post(route('services.dashboard.service-provider.businessschedules.store', [this.getSelectedModuleValue(), this.business.uuid]), this.form)
                    .then((response) => {
                        this.reset();
                ...
                window.axios.put(route('services.dashboard.service-provider.businessschedules.update', [this.getSelectedModuleValue(), this.form.id, this.business.uuid]), this.form)
                    .then((response) => {
                        this.reset();
                        this.businessSchedule = response.data.schedule;
                        this.$emit('childToParent', this.businessSchedule);
    ...
    mounted() {
        this.emitter.on('business-schedule-model', (args) => {
            this.form = useForm({
                ...
                open_at: args.schedule.open_at ? args.schedule.open_at : 'hh:mm A',
                close_at: args.schedule.close_at ? args.schedule.close_at : 'hh:mm A',
            });

<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Edit Service Provider`" :path="`Service Providers`" :subTitle="`Edit Service Provider`">
            </Breadcrumbs>
        </template>
        <!-- cover, thumbnail, logo section -->
        <div class="bg-white ps-2 pt-4 rounded">
            <image-section :business="business" :token="token" :mediaLogoSizes="mediaLogoSizes"
                :mediaThumbnailSizes="mediaThumbnailSizes" :mediaBannerSizes="mediaBannerSizes" :mediaSecondaryBannerSizes="mediaSecondaryBannerSizes"></image-section>
        </div>
        <div class="card pt-5">
                ...
                <form v-if="form" @submit.prevent="submit" ref="form">
                    <div class="row mb-2">
                        ...
                        <div class="fv-row mb-2 fv-plugins-icon-container"
                            ...
                            <Label for="Address"
                                :class="{ 'required': $page.props.auth.user.user_type == 'business_owner' }"
                                value="Address" />
                        ...
                        <div class="col-lg-4 fv-row mb-2 fv-plugins-icon-container">
                            <div class="form-check pt-5">
                                ...
                                <error :message="form.errors.virtual_appointments"></error>
                            </div>
                        </div>-->
                        <div class="col-lg-4 fv-row mb-2 fv-plugins-icon-container">
                            <div class="form-check pt-5">
...
export default {
    ...
    methods: {
        submit() {
            this.form.put(route('services.dashboard.service-provider.update', [this.getSelectedModuleValue(), this.business.uuid]), {
                errorBag: 'business',
    ...
    mounted() {
        this.form = useForm({
            ...
            home_delivery: this.business ? this.business.home_delivery ? true : false : false,
            virtual_appointments: this.business ? this.business.virtual_appointments ? true : false : false,

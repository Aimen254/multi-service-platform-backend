...
class ContactFormController extends Controller
{
    ...
    public function index($moduleId, $businessUuid)
    {
        ...
        return Inertia::render('Services::ContactForm/Index', [
            'forms' => $forms,
    ...
    public function show($moduleId, $businessUuid, $id)
    {
        try {
            ...
            return Inertia::render('Services::ContactForm/Show', [
    ...
    public function destroy($moduleId, $businessUuid, $id,Request $request)
    {
        try {
            ...
            if (auth()->user()->hasRole(['admin', 'newspaper', 'business_owner'])) {
                $contactForm = ContactForm::when(auth()->user()->hasRole('business_owner'), function ($query) {
            ...
            if ($currentCount > 1) {
                ...
                return Redirect::route('services.dashboard.service-provider.communication-portal.index', [$moduleId,$businessUuid, 'page' => $previousPage]);
            }
        } catch (ModelNotFoundException $e) {
            flash('Unable to find this communication portal', 'danger');
            return redirect()->back();

...
use Modules\Services\Http\Requests\BusinessRequest;
...
class BusinessController extends Controller
{
    ...
    public function index($moduleId)
    {
        ...
        return Inertia::render('Services::Business/Index', [
    ...
    public function create()
    {
        ...
        return Inertia::render('Services::Business/Create', [
            'businessOwners' => $businessOwners,
    ...
    public function store(BusinessRequest $request, $moduleId)
    {
        try {
            ...
            DB::commit();
            flash('Service Provider Added Sucessfully!', 'success');
            return \redirect()->route('services.dashboard.service-provider.index', $moduleId);
        } catch (\Exception $e) {
            Db::rollBack();
    ...
    {
        $mediaLogoSizes = \config()->get('services-module.media.logo');
        $mediaThumbnailSizes = \config()->get('services-module.media.thumbnail');
        $mediaBannerSizes = \config()->get('services-module.media.banner');
        $mediaSecondaryBannerSizes = \config()->get('services-module.media.secondaryBanner');
        $business = Business::with(['logo', 'thumbnail', 'banner', 'secondaryImages', 'secondaryBanner'])->where('uuid', $businessUuid)->first();
        $businessOwners = User::select(['id', DB::raw('CONCAT(id, \' - \', last_name) as text')])
            ->where('user_type', 'business_owner')->orWhere('user_type', 'customer')->where('status', 'active')->get();
        return Inertia::render('Services::Business/Edit', [
            'businessOwners' => $businessOwners,
            'is_admin' => request()->user()->user_type == 'admin' ? true : false,
            'business' => $business,
    ...
    public function update(BusinessRequest $request, $moduleId, $businessUuid)
    {
        try {
            ...
            DB::rollBack();
            flash('Unable to find this service provider', 'danger');
            return \redirect()->back();
        } catch (\Exception $e) {
            DB::rollBack();
            flash($e->getMessage(), 'danger');
    ...
    public function destroy($moduleId, $id,Request $request)
    {
        try {
            ...
            // dd(BusinessProducts::getProductCount($business));
            if (BusinessProducts::getProductCount($business)) {
                $business->delete();
                flash('Service Provider deleted succesfully', 'success');
                if ($currentCount > 1) {
                    return redirect()->back();
                } else {
                    $previousPage = max(1, $currentPage - 1);
                    return Redirect::route('services.dashboard.service-provider.index', [$moduleId, 'page' => $previousPage]);
                }
            } else {
                flash("We're unable to delete this Service Provider, as it has services associated with it.", 'danger');
            }
            return \redirect()->back();
        } catch (ModelNotFoundException $e) {
            flash('Unable to find this Service Provider', 'danger');
    ...
    public function changeStatus($moduleId, $id)
    {
        try {
            ...
            if ($business->status == 'inactive') {
                ...
                if (!$permission) {
                    ...
                    if (auth()->user()->user_type == 'admin') {
                        flash('Service Provider can not be activated due to subscription limitations.', 'danger');
                        return redirect()->back();
                    }
                    flash('You have activated maximum no of Service Provider according to your subscription plan.', 'danger', 'dashboard.subscription.subscribe.index');
                    return redirect()->back();
                }
            }
            if (!(auth()->user()->hasRole(['admin', 'newspaper'])) && ($business->status == 'inactive') && ($business->status_updated_by && $business->status_updated_by != auth()->user()->id)) {
                $message = "Service Provider is deactivated by administrators.";
                $status = "danger";
                DB::rollBack();
            } else {
                ...
                $business->save();
                DB::commit();
                $message = "Service Provider status changed succesfully.";
                $status = "success";
            }

<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Edit Service`" :path="`Posts - ${limit(form?.name, 36)}`" />
        </template>
                        ...
                        <div class="card-body py-3 row">
                            <div class="row">
                                ...
                                <div class="col-lg-6 py-2">
                                    <Label for="price" class="required" value="Salary" />
                                    <Input id="price" type="number" min="0"
                                        :class="{ 'is-invalid border border-danger': form.errors.price }"
                                        v-model="form.price" autofocus autocomplete="price" placeholder="Enter Salary" />
                                    <error :message="form.errors.price"></error>
                                </div>
                                <div class="col-lg-6 py-2">
                                    <Label for="stock" class="required" value="Salary Type" />
...
export default {
    ...
    methods: {
        submit() {
            this.form.put(route('employment.dashboard.employers.posts.update', [this.getSelectedModuleValue(), this.getSelectedBusinessValue(), this.product.uuid]), {
                errorBag: "product",
                preserveScroll: true,
            });
        },
        ...
        getLevelThreeTags(tag, level) {
            ...
            setTimeout(() => {
                var select2 = $(".select").select2({
                    ...
                    // ajax request
                    ajax: {
                        url: route("employment.dashboard.employers.post.tags", parameters),
                        dataType: "json",
                        delay: 250,
                ...
                // selected value
                window.axios.get(route('employment.dashboard.employers.post.tags', [this.getSelectedModuleValue(), this.getSelectedBusinessValue(), tag, level]), {
                    params: {

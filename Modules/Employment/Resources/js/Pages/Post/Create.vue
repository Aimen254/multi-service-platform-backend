<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Create Service`" :path="`Posts - Posts`" />
        </template>
...
export default {
    ...
    methods: {
        submit() {
            ...
            this.form.post(route('employment.dashboard.employers.posts.store', [this.getSelectedModuleValue(), this.getSelectedBusinessValue()]), {
                errorBag: "product",
                preserveScroll: true,
                onError: (errors) => {

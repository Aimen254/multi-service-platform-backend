<template>
    <Head title="Post Reviews" />
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Post Reviews`" :path="`Post -${ellipsis(product?.name)}`" />
        </template>
                ...
                <div class="card card-flush py-4">
                    <div class="card-header border-0 pt-5">
                        <h3 class="card-title align-items-start flex-column">
                            <h3 class="card-title align-items-start flex-column">
                                <EmploymentSearchInput :module="module" :callType="type" :searchedKeyword="searchedKeyword"
                                ...
                                <tbody v-if="reviews && reviews.data.length > 0">
                                    <template v-for="review in reviews.data" :key="review.id">
                                        <tr>
                                                ...
                                                <div class="d-flex align-items-center">
                                                    <div class="image-input image-input-empty">
                                                        <div class="image-input-wrapper w-40px h-40px"
                                                            :style="{ 'background-image': 'url(' + getImage(review.user.avatar) + ')' }" style="border-radius: 50%;">
                                                        </div>
                                                    </div>
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    <button permission="view_reviews" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" @click="openModal(review)"
                                                    ...
                                                    :url="route('employment.dashboard.post.reviews.destroy',[getSelectedModuleValue(),product.uuid, review.id])"
                    ...
                    <pagination :meta="reviews" :searchedKeyword="searchedKeyword" />
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
...
export default {
    ...
    methods: {
        openModal(review = null) {
            this.emitter.emit("employment-post-review-modal", {
                review: review,
            });
        },
    },

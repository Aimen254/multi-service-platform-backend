<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`All Businesses`" :path="`Businesses`" />
            ...
            <div class="me-4">
                ...
                <Filter :filterData="filterForm " :callType="type" :url="urlForFilter" :newKeyword="searchedKeyword"> 
                    ...
                    <div class="mb-10">
                        <label class="form-label fw-bold">Sort By Number of Orders:</label>
                        <div>
                            <select name="name" id="name" class="form-select form-select-solid text-muted form-select-sm" v-model="filterForm.sortByOrder">
                                <option value="desc" selected>Descending</option>
                                <option value="asc">Ascending</option>
                            </select>
                        </div>
                    ...
                    <div class="mb-10">
                        ...
                        <div class="mx-2">
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" value="inactive" v-model="filterForm.status">
                    ...
                    <div class="mb-10">
                        ...
                        <div class="mx-2">
                            <shapla-star-rating v-model="filterForm.reviewRating" :active-color="['#FFDF00']" :color="['#fede00']" class="fs-2 ps-1"/>
                        </div>
                    </div>
                </Filter> 
                <!--end::Menu-->
            </div>
            <Link :href="route('retail.dashboard.businesses.create', getSelectedModuleValue())" v-if="checkUserPermissions('add_business')" class="btn btn-sm btn-primary">
        ...
        <div class="card">
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <SearchInput :callType="type" :searchedKeyword="searchedKeyword" :filterFormData="filterForm " :module="'retail'"/>
                </h3>
            </div>
                    ...
                    <table class="table table-row-dashed align-middle gs-0 gy-4">
                        <thead>
                            <tr class="fw-bolder border-0 text-muted bg-light">
                                ...
                                <th class="min-w-120px">Business Owner</th>
                                <th class="min-w-120px">Rating</th>
                                <th class="min-w-120px">Tax</th>
                        ...
                        <tbody v-if="businesses && businesses.data.length > 0">
                            <template v-for="business in businesses.data" :key="business.id">
                                <tr>
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="d-flex justify-content-start flex-column mx-2">
                                                <span
                                                    ...
                                                    <span v-if="business.is_featured" class="text-muted fw-bold text-muted d-block fs-7">
                                    ...
                                    <td>
                                        <span v-if="business.business_owner" class="badge badge-light-primary fs-7 fw-bold text-capitalize"> {{business.business_owner.first_name}} {{business.business_owner.last_name}} </span>
                                    </td>
                                    <td>
                                        <span class="text-muted fw-bold text-muted d-block fs-7">
                                            <shapla-star-rating v-model="business.reviews_avg" :is-static="true" :active-color="['#ffad0f']" :color="['#ffad0f']" :padding="['4']"/>
                                        </span>
                                    </td>
                                    <td>
                                        <span class="text-muted fw-bold text-muted d-block fs-7">
                                            {{ business.tax }} <span class="fw-bold text-muted fs-9">$</span>
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge fs-7 fw-bold text-capitalize"
                                            :class="{ 'badge-light-success': business.status == 'active' ||  business.status === 'approved', 'badge-light-danger': business.status === 'inactive' ||  business.status === 'pending for approval' ||  business.status === 'rejected'}">{{ business.status }}
                                        </span>
                                    </td>
                                    <td>
                                        <span
                                            class="text-muted fw-bold text-muted d-block fs-7">{{ formatDate(business.created_at)}}</span>
                                    </td>
                                    <td>
                                        <div class="d-flex">
                                            <edit-section v-if="getSelectedModuleValue() == 'settings'"
                                                iconType="modal"
                                            ...
                                            <delete-section permission="delete_business"
                                                :url="route('retail.dashboard.businesses.destroy', [getSelectedModuleValue(), business.uuid])"
    ...
    <business-module-tag-modal></business-module-tag-modal>
</template>
...
import BusinessModuleTagModal from "./BusinesModuleTagModal.vue"
export default {
    components: {
        AuthenticatedLayout,
        Head,
        ...
        DeleteSection,
        EditSection,
        BusinessModuleTagModal
    },
    props: ['businessList', 'moduleTags', 'searchedKeyword', 'orderBy', 'status', 'rating', 'noOfOrders'],
    data () {
        return {
            businesses: null,
            type: 'business',
            logoUrl:'',
            urlForFilter: 'retail.dashboard.businesses.index',
            filterForm  : {
                orderBy : this.orderBy ? this.orderBy : 'desc',
                status : this.status ? this.status : null,
                reviewRating: this.rating ? this.rating : null,
                sortByOrder: this.noOfOrders ? this.noOfOrders : null,
            },
    ...
    methods: {
        changeStatus (business) {
            ...
            this.swal.fire({
                ...
                if (result.value) {
                    showWaitDialog()
                    this.$inertia.visit(route('retail.dashboard.business.status', [this.getSelectedModuleValue(), business.id]), {

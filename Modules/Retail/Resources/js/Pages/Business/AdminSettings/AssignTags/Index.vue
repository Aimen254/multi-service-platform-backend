<template>
    ...
    <BreezeAuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Business Tags`" :path="`Businesses - Admin Settings`" />
        </template>
                            ...
                            <form class="form" v-if="form" @submit.prevent="assignGlobalTags">
                                <div class="col-md-12">
                                    <div v-if="industryTagsList.length">
                                        ...
                                        <TagComponent placeholder="Level two tags" :uuid="business.uuid" :assignedTags="industryTags" :autocomplete="industryTagsList" :onlyFromAutoComplete="true" v-if="industryTags" :tagType="'industry_tags'"/>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-12" v-if="industryTags.length > 0">
                                    <div>
                                        ...
                                        <TagComponent placeholder="Level three tags" @fetchSuggestions="fetchSuggestions()" :uuid="business.uuid" :assignedTags="productTags" type="product" :autocomplete="this.productTagList" :onlyFromAutoComplete="true" :tagType="'product_tags'" :openOnClick="true" :level="2"   />
                                    </div>
                                </div>
...
export default {
    ...
    methods: {
        assignGlobalTags(id) {
            ...
            this.form.post(route("retail.dashboard.business.assign.standard-tags", [this
        ...
        removeProductTags(tags,tagType) {
             this.processing=true;
             this.pendingRequests++;
            window.axios.post(route('retail.dashboard.business.remove-product-tags', [this.getSelectedModuleValue(), this.business.uuid]) ,
            {
            ...
            .catch(error => {
                if(tagType == "industry_tag"){
                    this.industryTags.push(tags.tag);
                }else if(tagType == "product_tag"){
                    this.productTags.push(tags.tag);
                }
                console.log(error)
                this.$notify({
                            group: "toast",
        ...
        fetchSuggestions() {
             window.axios.get(route('retail.dashboard.business.business-level-three-tags', [this.getSelectedModuleValue(), this.business.uuid]) ,
            {
                params: {
    ...
    mounted() {
        this.emitter.on('remove_product_tags', (args) => {
              if(args.tagType == "industry_tags"){
            this.removeProductTags(args.industry_tags,'industry_tag')
              }else if(args.tagType =="product_tags"){
            this.removeProductTags(args.industry_tags,'product_tag')
              }
        })

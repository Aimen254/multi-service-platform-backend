...
methods : {
    ...
    submit() {
        ...
        this.$inertia.post(route("retail.dashboard.product.media.update", [this.getSelectedModuleValue(), this.product.uuid, this.product.main_image ? this.product.main_image.id : 0]), this.form, {
            errorBag: "admin",
            preserveScroll: true,
            onError: (data) => {
...
mounted(){
    ...
    var submissionUrl = route('retail.dashboard.product.media.store', [moduleId, this.product.uuid])
    $("#product").dropzone({ 
        url: submissionUrl,
        parallelUploads:6,
        ...
        removedfile: function (file) {
            if (file.id) {
                window.swal.fire({
                    ...
                    if (result.value) {
                        showWaitDialog()
                        window.axios.delete(route('retail.dashboard.product.media.destroy', [moduleId, productMedia.uuid, file.id]))
                            .then((response) => {
                                hideWaitDialog()

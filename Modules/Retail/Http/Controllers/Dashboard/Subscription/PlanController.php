...
namespace Modules\Retail\Http\Controllers\Dashboard\Subscription;
...
class PlanController extends Controller
{
    ...
    public function index($moduleId)
    {
        ...
        return Inertia::render('Retail::Subscription/Plans/Index', [
    ...
    {
        return Inertia::render('Retail::Subscription/Plans/Edit', []);
    }
    ...
    public function update(subscriptionPlanRequest $request, $moduleId, $id)
    {
        try {
            ...
            return \redirect()->route('retail.dashboard.subscription.plan.index', $moduleId);
        } catch (\Stripe\Exception\CardException $e) {
            flash($e->getMessage(), 'danger');
            return \redirect()->back();
        } catch (\Stripe\Exception\InvalidRequestException $e) {
    ...
    public function destroy($moduleId, $id, Request $request)
    {
        try {
            ...
            if ($currentCount > 1) {
                ...
                return Redirect::route('retail.dashboard.subscription.plan.index', [$moduleId, 'page' => $previousPage]);
            }
        } catch (\Stripe\Exception\CardException $e) {
            flash($e->getMessage(), 'danger');
            return \redirect()->back();

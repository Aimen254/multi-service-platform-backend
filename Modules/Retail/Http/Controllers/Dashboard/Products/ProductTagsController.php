...
use App\Models\ProductIgnoredTags;
...
class ProductTagsController extends Controller
{
    ...
    public function index($moduleId, $uuid)
    {
        try {
            ...
            $ignoredOrphanTags = $product->tags()->asTag()->whereHas('standardTags_', function ($query) {
                $query->where('priority', '<>', 1);
            })->orWhereDoesntHave('standardTags_')->where('product_id', $product->id)
                ->get()->reject(function ($record) {
                    $tag = $record->whereHas('standardTags_', function ($query) use ($record) {
                        $query->where('priority', '<>', 1)->where('slug', $record->slug);
                    })->first();
            ...
            $ignoredStandardTags = $product->standardTags()->asTag()->where('priority', '<>', 1)->where('type', '<>', 'module')->get();
            ...
            return Inertia::render('Retail::Products/Settings/AssignTags', [
                'product' => $product,
    ...
    public function assignTags($moduleId, $uuid)
    {
        try {
            ...
            $previousbrandTags = $product->standardTags()->where('type', 'brand')->pluck('id')->toArray();
            $removeBrandTags = array_diff($previousbrandTags, $brandTags);
            $product->standardTags()->detach($removeBrandTags);
            ...
            ProductTagsLevelManager::priorityThree($product, null, false, $brandTags, null, count($removeBrandTags) > 0 ? $removeBrandTags : null);
            DB::commit();
            flash('Product tags updated successfully.', 'success');
            return \redirect()->back();

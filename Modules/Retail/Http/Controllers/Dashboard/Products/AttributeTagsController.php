...
class AttributeTagsController extends Controller
{
    ...
    public function index($moduleId, $uuid)
    {
        try {
            ...
            $standardTags = StandardTag::whereType('attribute')->with('attribute')->asTag()->active()->get();
            ...
            return Inertia::render('Retail::Products/Settings/AttributeTags', [
                'product' => $product,
                'assignedTag' => $assignedTags,
                // 'allAttributeTags' => $allAttributeTags,
                'attributes' => Attribute::active()->whereHas('moduleTags', function ($query) use ($moduleId) {
    ...
    public function assignTags($moduleId, $uuid)
    {
        try {
            ...
            DB::commit();
            flash('Product attribute tags updated successfully.', 'success');
            return \redirect()->back();

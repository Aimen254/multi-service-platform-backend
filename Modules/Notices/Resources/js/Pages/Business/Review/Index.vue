<template>
    ...
    <AuthenticatedLayout>
        ...
        <template #breadcrumbs>
            <Breadcrumbs :title="`Reviews`" :path="`Organizations`" />
        </template>
            ...
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bolder fs-3 mb-1">
                        <NoticesSearchInput :business="business" :callType="type" :searchedKeyword="searchedKeyword" />
                    </span>
                </h3>
            </div>
            <!--end::Header-->
                        ...
                        <tbody v-if="reviews && reviews.data.length > 0">
                            <template v-for="review in reviews.data" :key="review.id">
                                <tr>
                                    ...
                                    <td>
                                        <span class="text-muted fw-bold text-muted d-block fs-7" data-bs-toggle="tooltip"
                                            data-bs-placement="bottom" :data-bs-original-title="review.comment">{{
                                    ...
                                    <td>
                                        <span class="badge fs-7 fw-bold text-capitalize"
                                            :class="{ 'badge-light-success': review.status == 'active', 'badge-light-danger': review.status === 'inactive' }">{{
                                                review.status }}</span>
                                    </td>
                                    <td>
                                        <span class="text-muted fw-bold text-muted d-block fs-7">{{
                                            formatDate(review.created_at) ? formatDate(review.created_at) : 'Null' }}</span>
                                    </td>
                                    <td>
                                        <div class="d-flex">
                                            ...
                                            <button permission="view_comment"
                                                ...
                                                @click="openModal(review)" data-bs-toggle="tooltip" data-bs-trigger="hover"
                                                data-bs-dismiss="click" data-bs-placement="bottom"
                                                data-bs-original-title="view"><i class="fas fa-eye"></i></button>
                                            <delete-section permission="delete_reviews"
                                                :url="route('notices.dashboard.organization.reviews.destroy', [getSelectedModuleValue(), review.id, business.id])" 
...
import NoticesSearchInput from '../../../Components/NoticesSearchInput.vue'
        ...
        changeStatus(id) {
            this.swal.fire({
                ...
                if (result.value) {
                    ...
                    this.$inertia.visit(route('notices.dashboard.organization.review.status', [this.getSelectedModuleValue(), id, this.business.id]), {

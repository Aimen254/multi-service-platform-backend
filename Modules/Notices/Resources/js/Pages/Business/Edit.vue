<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Edit Organization`" :path="`Organization`" :subTitle="`Edit Organization`">
            </Breadcrumbs>
        </template>
        ...
        <div class="bg-white ps-2 pt-4 rounded">
            <image-section :business="business" :token="token" :mediaLogoSizes="mediaLogoSizes"
                :mediaThumbnailSizes="mediaThumbnailSizes" :mediaBannerSizes="mediaBannerSizes"></image-section>
        </div>
        <div class="card pt-5">
                ...
                <form v-if="form" @submit.prevent="submit" ref="form">
                    <div class="row mb-2">
                        ...
                        <div class="col-lg-12 fv-row mb-2 fv-plugins-icon-container" v-if="is_admin">
                            <Label for="Dealership Owner" class="required" value="Service Organization Owner" />
                            <select2 v-model="form.owner_id" :options="Owners"
                                :class="{ 'is-invalid border border-danger': form.errors.owner_id }"
                                placeholder="Select Service Organization Owner">
                            </select2>
...
export default {
    ...
    methods: {
        submit() {
            this.form.put(route('notices.dashboard.organizations.update', [this.getSelectedModuleValue(), this.business.uuid]), {
                errorBag: 'business',

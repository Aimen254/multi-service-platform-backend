<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Notices`" :path="`Notices`" />
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                ...
                <Link v-if="checkUserPermissions('add_products')"
                    :href="route('notices.dashboard.organization.notices.create', [getSelectedModuleValue(), getSelectedBusinessValue()])"
        ...
        <div class="card">
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <notices-search-input-vue :module="module" :callType="type" :searchedKeyword="searchedKeyword"
                    ...
                    <table class="table table-row-dashed align-middle gs-0 gy-4">
                        <thead>
                            <tr class="fw-bolder border-0 text-muted bg-light">
                                <th class="ps-4 min-w-120px rounded-start">Description</th>
                        ...
                        <tbody v-if="products && products.data.length > 0">
                            <template v-for="product in  products.data " :key="product.id">
                                <tr :style="[product.status == 'tags_error' ? 'background:#ffe6e6' : null]">
                                    <td class="px-4">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="d-flex justify-content-start flex-column mx-2">
                                                <span class="text-dark fw-bolder text-hover-primary mb-1 fs-6"
                                                    ...
                                                    v-html="ellipsis(product.description)">
                                                </span>
                                    ...
                                    <td>
                                        <span class="text-muted fw-bold text-muted d-block fs-7 text-capitalize">
                                            <span class="fw-normal text-muted fs-7">{{ product.type }}</span>
                                        </span>
                                    </td>
                                    <td>
                                        <span
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            ...
                                            <edit-section iconType="link" permission="edit_products"
                                                :url="route('notices.dashboard.organization.notices.edit', [getSelectedModuleValue(), getSelectedBusinessValue(), product.uuid])" />
                                            <delete-section permission="delete_products"
                                                :url="route('notices.dashboard.organization.notices.destroy', [getSelectedModuleValue(), getSelectedBusinessValue(), product.uuid])" 
...
import NoticesSearchInputVue from '../../Components/NoticesSearchInput.vue'
...
export default {
    components: {
        ...
        Breadcrumbs,
        Filter,
        NoticesSearchInputVue
    },

    props: ['productsList', 'searchedKeyword', 'orderBy', 'status', 'standardTags', 'tag'],
    data() {
        return {
            products: this.productsList,
            type: 'notices',
            urlForFilter: 'notices.dashboard.organization.notices.index',
            filterForm: {
        ...
        changeStatus(uuid) {
            this.swal.fire({
                ...
                if (result.value) {
                    ...
                    this.$inertia.visit(route('notices.dashboard.organization.notices.status', [this.getSelectedModuleValue(), this.getSelectedBusinessValue(), uuid]), {

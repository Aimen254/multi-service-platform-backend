<template>
    <Head title="Notice Images" />
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Notice Images`" :path="`Notices - ${product?.name}`" />
        </template>
                        ...
                        <div class="fv-row mb-4">
                            <div class="image-input image-input-outline image-input-empty" data-kt-image-input="true">
                                <div class="image-input-wrapper w-125px h-125px"
                                    :style="{ 'background-image': 'url(' + getImage(url, isSaved, 'product', product.main_image ? product.main_image.is_external : 0) + ')' }">
                                </div>
...
export default {
    ...
    methods: {
        ...
        submit() {
            ...
            this.$inertia.post(route("notices.dashboard.notices.media.update", [this.getSelectedModuleValue(), this.product.uuid, this.product.main_image ? this.product.main_image.id : 0]), this.form, {
                errorBag: "admin",
                preserveScroll: true,
                onError: (data) => {
    ...
    mounted() {
        ...
        var submissionUrl = route('notices.dashboard.notices.media.store', [moduleId, this.product.uuid])
        $("#product").dropzone({
            url: submissionUrl,
            parallelUploads: 6,
            ...
            removedfile: function (file) {
                if (file.id) {
                    window.swal.fire({
                        ...
                        if (result.value) {
                            showWaitDialog()
                            window.axios.delete(route('notices.dashboard.notices.media.destroy', [moduleId, productMedia.uuid, file.id]))
                                .then((response) => {
                                    hideWaitDialog()

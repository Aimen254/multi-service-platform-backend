...
use Modules\Boats\Http\Requests\DealershipRequest;
...
class DealershipController extends Controller
{
    ...
    public function index($moduleId)
    {
        ...
        return Inertia::render('Boats::Dealership/Index', [
    ...
    public function create($moduleId)
    {
        ...
        return Inertia::render('Boats::Dealership/Create', [
            'businessOwners' => $businessOwners,
     ...
     * @return Renderable
     */
    public function store(DealershipRequest $request, $moduleId)
    {
        try {
            DB::beginTransaction();
            ...
            return \redirect()->route('boats.dashboard.dealership.index', $moduleId);
        } catch (\Exception $e) {
            Db::rollBack();
    ...
    public function edit($moduleId, $businessUuid)
    {
        ...
        $businessOwners = User::select(['id', DB::raw('CONCAT(id, \' - \', last_name) as text')])
            ->where('user_type', 'business_owner')
            ->where('status', 'active')->get();
        return Inertia::render('Boats::Dealership/Edit', [
            'businessOwners' => $businessOwners,
            'is_admin' => request()->user()->user_type == 'admin' ? true : false,
            'business' => $business,
     ...
     * @return Renderable
     */
    public function update(DealershipRequest $request, $moduleId, $businessUuid)
    {
        try {
            DB::beginTransaction();
        ...
        try {
            ...
            if (BusinessProducts::getProductCount($business)) {
                ...
                if ($currentCount > 1) {
                    ...
                    return Redirect::route('boats.dashboard.dealership.index', [$moduleId, 'page' => $previousPage]);
                }
            } else {
    ...
    public function changeStatus($moduleId, $id)
    {
        try {
            ...
            //             flash('Dealership can not be activated due to subscription limitations.', 'danger');
            //             return redirect()->back();
            //         }
            //         flash('You have activated maximum no of dealerships according to your subscription plan.', 'danger', 'dashboard.subscription.subscribe.index');
            //         return redirect()->back();
            //     }
            // }
            if (!(auth()->user()->hasRole(['admin', 'newspaper'])) && ($business->status == 'inactive') && ($business->status_updated_by && $business->status_updated_by != auth()->user()->id)) {
                $message = "Dealership is deactivated by administrators.";
                $status = "danger";
                DB::rollBack();
            } else {
                ...
                $business->save();
                DB::commit();
                $message = "Dealership status changed succesfully.";
                $status = "success";
            }

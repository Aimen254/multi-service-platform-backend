<template>
        ...
        <div class="row">
          ...
          <div class="col-md-10">
            ...
            <div class="fv-row mb-4">
              ...
              <p class="d-inline-block fs-9 text-muted pt-2">Image must be {{mediaBannerSizes.width}} x {{mediaBannerSizes.height}}</p>
              <p class="d-inline-block fs-9 pt-2 text-muted float-end">Dealership Page Banner</p>
            </div>
            <error v-if="form && form.type == 'banner'" :message="form.errors.image"></error>
            <div class="fv-row mb-4">
              <div class="image-input image-input-outline image-input-empty w-100"
                data-kt-image-input="true">
                <div
                  class="image-input-wrapper w-100 h-350px"
                  :style="{ 'background-image': 'url(' + getImage(business.secondary_banner ? business.secondary_banner.path : null , true, 'secondaryBanner') + ')' }"
                  ></div>
                <EditImage :title="'Change Secondary Banner'" @click="openFileDialog('secondaryBanner')">
                </EditImage>
                <input id="secondaryBanner" type="file" class="d-none"
                    :accept=" accept" @change="onFileChange($event, 'secondaryBanner')"
                />
                <RemoveImageButton v-if="url" :title="'Remove Banner'"
                  @click="removeImage(business.secondary_banner.id, 'secondaryBanner')" />
              </div>
              <p class="d-inline-block fs-9 text-muted pt-2">Image must be {{mediaSecondaryBannerSizes.width}} x {{mediaSecondaryBannerSizes.height}}</p>
              <p class="d-inline-block fs-9 pt-2 text-muted float-end">Dealership Listing Banner</p>
            </div>
            <error v-if="form && form.type == 'secondaryBanner'" :message="form.errors.image"></error>
          </div>
        </div>
        <div class="row mt-2">
          <div class="col-lg-12">
  ...
  export default {
    props: ["business", "mediaLogoSizes", "mediaThumbnailSizes", "mediaBannerSizes","mediaSecondaryBannerSizes", "token"],
    components: {
        RemoveImageButton,
        EditImage,
        Error,
    ...
    methods: {
      ...
      setImage(business, type) {
        ...
        }else if (type == "secondaryBanner") {
          this.url = business.secondary_banner ? business.secondary_banner.path : "";
          this.isSaved = this.url ? true : false;
          return this.getImage(this.url, this.isSaved, "secondaryBanner");
        } else {
    ...
    mounted() {
      ...
      $("#business").dropzone({ 
          ...
          error: function(data) {
              ...
              $('#business').find('div.dz-processing').remove();
              document.getElementById('upload_icon') ? document.getElementById('upload_icon').style.display = 'block' : null
              document.getElementById('text_message') ? document.getElementById('text_message').style.display = 'block' : null
          ...
          init: function() {
              ...
              businessDropzone.on("addedfiles", function(files) {
                ...
                if(count > 3) {
                  ...
                  document.getElementById('upload_icon') ? document.getElementById('upload_icon').style.display = 'none' : null
                  document.getElementById('text_message') ? document.getElementById('text_message').style.display = 'none' : null

<template>
    ...
    <AuthenticatedLayout>
        <template #breadcrumbs>
            <Breadcrumbs :title="`Administrators`" :path="`Users`"></Breadcrumbs>
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <Link v-if="checkUserPermissions('add_administrators')"
                    :href="route('dashboard.administrators.create')"
        ...
        <div class="card">
            ...
            <div class="card-body py-3">
                <div class="table-responsive">
                    <table class="table table-row-dashed align-middle gs-0 gy-4">
                        ...
                        <tbody v-if="administrators && administrators.data.length > 0">
                            <template v-for="administrator in administrators.data" :key="administrator.id">
                                <tr>
                                    <td class="px-4">
                                        <div class="d-flex align-items-center">
                                            <div class="image-input image-input-empty">
                                                ...
                                                :style="{ 'background-image': 'url(' + getImage(administrator.avatar, true, 'avatar') + ')' }"
                                                ></div>
                                            </div>
                                    ...
                                    <td>
                                        <div class="d-flex">
                                            <Toggle v-if="checkUserPermissions('edit_administrators')"
                                                :status="booleanStatusValue(administrator.status)"
                                                @click.prevent="changeStatus(administrator.id)"
                                            />
                                            <edit-section iconType="link" 
                                                permission="edit_administrators"
                                                :url="route('dashboard.administrators.edit', administrator.id)"/>
                                            <delete-section permission="delete_administrators"
                                                :url="route('dashboard.administrators.destroy', administrator.id)" 
                                                :currentPage="administrators.current_page" :currentCount="administrators.data.length"/>
                                        </div>
                                    </td>
                                </tr>
                            </template>
            ...
            <pagination :meta="administrators" :keyword="searchedKeyword"/>
        </div>
    </AuthenticatedLayout>
</template>
...
export default {
    ...
    data () {
        return {
            administrators: this.administratorsList,
            type: 'administrator'
        }
    },
    watch: {
        administratorsList: {
            handler(administratorsList) {
                this.administrators = administratorsList
            },
            deep: true
        }
    },
        ...
        changeStatus (id) {
            this.swal.fire({
                ...
                if (result.value) {
                    ...
                    this.$inertia.visit(route('dashboard.administrator.change.status', id), {

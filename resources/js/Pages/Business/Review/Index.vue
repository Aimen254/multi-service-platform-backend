<template>
    ...
    <AuthenticatedLayout>
        ...
        <template #breadcrumbs>
            <Breadcrumbs :title="`Reviews`" :path="`Businesses`" />
        </template>
            ...
            <div class="card-header border-0 pt-5">
                <h3 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bolder fs-3 mb-1">
                        <SearchInput :business="business" :callType="type" :searchedKeyword="searchedKeyword" />
                    </span>
                </h3>
            </div>
            <!--end::Header-->
                        ...
                        <tbody v-if="reviews && reviews.data.length > 0">
                            <template v-for="review in reviews.data" :key="review.id">
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="text-muted fw-bold text-muted d-block fs-7">{{ formatDate(review.created_at)}}</span>
                                    </td>
                                    <td>
                                        <div class="d-flex">
                                           <Toggle v-if="checkUserPermissions('edit_reviews')"
                                                ...
                                                :url="route('dashboard.business.reviews.destroy', [review.id, business.id])" 
        ...
        changeStatus(id) {
            this.swal.fire({
                ...
                if (result.value) {
                    ...
                    this.$inertia.visit(route('dashboard.business.review.status', [id, this.business.id]), {
